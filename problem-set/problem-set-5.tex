...
\begin{questions}
	\question[20] \textbf{Threat Models and Key Management}\topicbadge{2.4}

	\begin{parts}
		\part[10] \textbf{The Mud Puddle Test:}
		...
		\begin{subparts}
			...
			\subpart Your CEO insists on having a ``password recovery'' feature. Design two different approaches that balance security with usability, clearly stating the security trade-offs of each.
			\subpart Compare Apple's Advanced Data Protection approach with a mandatory E2EE approach. Which would you recommend for the healthcare provider and why?
		\end{subparts}

		\part[10] \textbf{Multi-Device Key Management:}
	...
	\question[15] \textbf{Authentication Protocols}\topicbadge{2.4}

	\begin{parts}
		\part[15] \textbf{OPAQUE and Password Authentication:}
		You're evaluating authentication mechanisms for a new E2EE cloud storage service.
		\begin{subparts}
			\subpart Compare traditional password hashing (PBKDF2/Argon2) with OPAQUE. For each approach, explain what the server learns about the user's password and what attacks are possible after server compromise.
			\subpart The OPAQUE protocol uses an OPRF. Explain how the 2HashDH OPRF works and why the server cannot perform offline dictionary attacks.
			\subpart Your team suggests using OPAQUE's shared session key for encrypting the backup key. Explain why this would be catastrophic for security and what should be used instead.
		\end{subparts}
	\end{parts}
\end{questions}
...
\begin{questions}
	\question[20] \textbf{MEGA Attack Analysis}\topicbadge{2.4}
	...
	\begin{parts}
		...
		\begin{subparts}
			...
			\subpart The attack exploits MEGA's RSA-CRT implementation. Describe how modifying the parameter $u' \neq u = q^{-1} \bmod p$ creates an oracle that leaks information about the prime factor $q$.
			\subpart MEGA attempted to patch these vulnerabilities but introduced new ones. Explain why ad hoc fixes to broken cryptographic designs are dangerous.
		\end{subparts}

		\part[10] \textbf{Plaintext Recovery:}
		...
		\begin{subparts}
			...
			\subpart Design a simpler, more secure key hierarchy that would prevent these cascading failures.
		\end{subparts}
	\end{parts}

	\question[15] \textbf{Nextcloud Vulnerabilities}\topicbadge{2.4}

	\begin{parts}
		\part[7] \textbf{Authentication Failures:}
		Nextcloud's E2EE implementation has critical authentication flaws.
		\begin{subparts}
			\subpart Explain the ``Key Insertion Attack,'' where the server can substitute metadata keys. Why doesn't RSA-OAEP provide authentication?
			\subpart Describe the ``Ghost Key Attack'' involving missing bounds checking and propose fixes for both attacks.
		\end{subparts}
...
\begin{questions}
	\question[15] \textbf{HSM-Based Key Recovery}\topicbadge{2.4}

	\begin{parts}
		\part[15] \textbf{System Design and OPAQUE:}
		...
		\begin{subparts}
			...
			\subpart Compare WhatsApp's centralized HSM approach with Signal's SVR3 system, which distributes trust across multiple enclaves. What are the advantages of Signal's approach?
			\subpart The HSM enforces attempt limits to prevent brute-force attacks. Calculate how many attempts an attacker would need on average to crack a 6-digit PIN versus a 15-character password.
		\end{subparts}
	\end{parts}
\end{questions}
...
\begin{questions}
	\question[15] \textbf{Formal Protocol Design}\topicbadge{2.4}

	\begin{parts}
		\part[8] \textbf{Building a Secure Protocol:}
		...
		\part[7] \textbf{Implementation Considerations:}
...
\begin{tcolorbox}[colframe=EarthBrown!30!white,colback=EarthBrown!5!white]
	\section*{Bonus Question\hfill\topicpill{2.4}}
	\begin{questions}
		\bonusquestion[10] \textbf{The Cungadero Cloud Deluxe Audit}
