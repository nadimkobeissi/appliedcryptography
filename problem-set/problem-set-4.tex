...
\begin{questions}
	\question[20] \textbf{TLS Attack Analysis}

	\begin{parts}
		\part[10] \textbf{The Legacy Downgrade Scenario:}
		...
		\begin{subparts}
			...
			\subpart Your CEO asks: ``Can't we just keep supporting old protocols for compatibility?'' Write a technical risk assessment explaining why this is dangerous, using specific examples from the TLS attack timeline.
		\end{subparts}

		\part[10] \textbf{Certificate Authority Compromise:}
		...
		\begin{subparts}
			...
			\subpart Analyze the trade-offs between the CA model and alternative approaches like blockchain-based certificates or DANE. What would need to change for these alternatives to become practical?
		\end{subparts}
	\end{parts}

	\question[15] \textbf{TLS 1.3 Design Decisions}

	\begin{parts}
		\part[7.5] \textbf{Forward Secrecy vs Performance:}
		...
		\begin{subparts}
			...
			\subpart Analyze the security trade-offs of 0-RTT resumption. Design a policy for when your service should accept 0-RTT data and when it should refuse it.
			\subpart Your infrastructure team wants to use RSA key exchange for ``simplicity.'' Explain why ephemeral Diffie-Hellman is crucial for forward secrecy, using concrete attack scenarios. Can RSA be used for ephemeral key exchange instead of Diffie-Hellman? Explain your answer.
		\end{subparts}

		\part[7.5] \textbf{Enterprise Monitoring and Middleboxes:}
		...
		\begin{subparts}
			...
			\subpart Design a solution that balances enterprise security requirements with user privacy. Consider split-tunneling, certificate pinning exceptions, and audit logging. How do you prevent abuse while enabling legitimate monitoring?
			\subpart Analyze the ``TLS Encrypted Client Hello'' (ECH) extension. How does it further complicate enterprise monitoring? Propose a deployment strategy that addresses both privacy advocates' and enterprise administrators' concerns.
		\end{subparts}
	\end{parts}
\end{questions}
...
\begin{questions}
	...
	\begin{parts}
		...
		\begin{subparts}
			...
			\subpart The company argues that their network has low traffic, making statistical attacks impractical. Describe how active attacks (packet injection) can accelerate key recovery.
		\end{subparts}
		...
		\begin{subparts}
			...
			\subpart Design a broadcast attack scenario against this VPN protocol, assuming you can observe the same message encrypted multiple times. How would you exploit RC4's statistical biases?
	...
	\begin{parts}
		...
		\begin{subparts}
			...
			\subpart Your legacy product team argues that supporting old algorithms is necessary for backward compatibility. Develop a risk-based approach to determine when compatibility must be sacrificed for security.
			\subpart Create a communication strategy for informing users about algorithm deprecation. How do you balance transparency about vulnerabilities with avoiding panic?
		\end{subparts}
	\end{parts}
\end{questions}
...
\begin{questions}
	\question[20] \textbf{Protocol Design and Analysis}

	\begin{parts}

		\part[10] \textbf{Designing a Secure Handshake:}
		\begin{subparts}
			\subpart Design a complete key exchange protocol for a secure messenger. Your design should address:
			\begin{itemize}
			...
			\end{itemize}
			\subpart Analyze the security properties of your protocol. What attacks does it defend against, and what are its limitations?
			\subpart Discuss how your protocol would handle practical issues like key fingerprint verification and key rotation.
		\end{subparts}
		...
		\begin{subparts}
			...
			\subpart Design a hybrid system that combines PGP's explicit trust model with Signal's usability. What compromises are necessary?
		\end{subparts}
		...
		\begin{subparts}
			...
			\subpart Signal's code is open source but lacks reproducible builds. Design a verification strategy that helps users determine if their client matches the published source. What are the limitations of this approach, and what attacks remain undetectable?
			\subpart Evaluate the risk profile for different user groups (casual users, journalists, activists) if Whistletaker implements subtle backdoors. Consider: selective targeting, parallel construction risks, and the difference between passive monitoring and active attacks. What alternative architectures or protocols would provide better resistance to operator compromise?
		\end{subparts}
		...
		\begin{subparts}
			...
			\subpart Analyze the security implications of storing pre-keys on a server. How does Signal minimize the trust required in the server?
	...
	\question[30] \textbf{Authenticated Key Exchange and Ratcheting Analysis}

	\begin{parts}
		\part[6] \textbf{Basic Authentication Properties:}
		...
		\part[8] \textbf{Identity Binding and Forward Secrecy:}
		...
		\part[16] \textbf{Broken Ratcheting Protocols:}
...
\begin{tcolorbox}[colframe=EarthBrown!30!white,colback=EarthBrown!5!white]
	...
	\begin{questions}
		\bonusquestion[30] The evolution from SSL to TLS 1.3 represents one of the most important case studies in applied cryptography. Choose one of the following research topics:
		\begin{parts}
