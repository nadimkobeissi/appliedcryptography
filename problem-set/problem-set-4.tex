...
\begin{questions}
	\question[20] \textbf{TLS Attack Analysis}\topicbadge{2.1}

	\begin{parts}
		\part[10] \textbf{The Legacy Downgrade Scenario:}
		You are the security engineer for a major e-commerce platform. Your security scan reveals that 15\% of your customers are still using browsers that only support SSL 3.0 and TLS 1.0.
		\begin{subparts}
	...
	\question[15] \textbf{TLS 1.3 Design Decisions}\topicbadge{2.1}

	\begin{parts}
		\part[7] \textbf{Forward Secrecy vs Performance:}
		...
		\part[8] \textbf{Enterprise Monitoring and Middleboxes:}
...
\begin{questions}
	\question[20] \textbf{Stream Cipher Vulnerabilities}\topicbadge{2.2}

	\begin{parts}
		\part[10] \textbf{WEP Forensics Challenge:}
		...
		\part[10] \textbf{RC4 in Modern Protocols:}
		...
		\begin{subparts}
			...
			\subpart Compare RC4's failure modes to those of modern stream ciphers like ChaCha20. What design principles make ChaCha20 resistant to the attacks that broke RC4?
		\end{subparts}
	\end{parts}
\end{questions}
...
\begin{questions}
	\question[15] \textbf{Protocol Design and Analysis}\topicbadge{2.3}
	...
	\begin{parts}
		...
		\begin{subparts}
			\subpart Compare the threat models of PGP and Signal. What assumptions does each make about user behavior, key management, and trust establishment?
	...
	\question[30] \textbf{Authenticated Key Exchange and Ratcheting Analysis}\topicbadge{2.3}

	\begin{parts}
		\part[7] \textbf{Basic Authentication Properties:}
		...
		The following protocol attempts to provide authenticated key exchange. Determine whether it is vulnerable to identity misbinding, key compromise impersonation (KCI), or replay attacks, and whether it lacks perfect forward secrecy.
		...
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		Here, $a$ is $A$'s long-term private key, $b$ is $B$'s long-term private key, and the session key is $K = g^{xy}$.

		\part[15] \textbf{Broken Ratcheting Protocols:}
		...
		\begin{itemize}
			...
			\item DH ratchet on every message:
			      \begin{itemize}
				      ...
				      \item Shared secret: $ss = g^{e_i \cdot b}$
...
\begin{tcolorbox}[colframe=EarthBrown!30!white,colback=EarthBrown!5!white]
	\section*{Bonus Question\hfill\topicpill{2.1}}
	\begin{questions}
		...
		\begin{parts}
			...
			\part \textbf{Messaging Protocol Convergence}: Compare the security properties achieved by Signal, MLS, and the latest WhatsApp protocols. Are we converging on an ``optimal'' design for secure messaging? What fundamental trade-offs remain unresolved?
		\end{parts}
