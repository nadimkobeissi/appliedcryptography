...
\begin{questions}
	\question[20] \textbf{The Schnorr Protocol}\topicbadge{2.8}
	...
	\begin{parts}
		...
		\begin{subparts}
			\subpart Walk through one complete execution of the Schnorr protocol. Show all computations when Alice uses $y = 42$, the verifier challenges with $c = 7$, and Alice's secret is $a = 13$ (work modulo 101 for simplicity). Verify that the protocol accepts.
	...
	\question[15] \textbf{Soundness via Extraction}\topicbadge{2.8}

	You intercept two accepting transcripts from the same prover with the same commitment but different challenges: $(Y = g^{100}, c = 15, r = 355)$ and $(Y = g^{100}, c' = 22, r' = 474)$. The public key is $A = g^{17}$.
	\begin{parts}
		\part[10] Extract the secret $a$ from these two transcripts. Show your work.
		\part[5] Explain why this extraction technique implies that a cheating prover has at most $\frac{1}{n}$ probability of fooling the verifier, where $n$ is the number of possible challenges.
	\end{parts}
\end{questions}

\section{Sigma Protocols and Composition}
\begin{questions}
	\question[20] \textbf{Building Complex Proofs}\topicbadge{2.8}
...
\begin{questions}
	\question[20] \textbf{The Fiat-Shamir Transform}\topicbadge{2.8}
	...
	\question[15] \textbf{Circuits and General-Purpose ZK}\topicbadge{2.8}

	You want to prove knowledge of a value $x$ such that $\textsf{SHA-256}(x) = y$ for a given $y$.
	\begin{parts}
...
\begin{questions}
	\question[10] \textbf{Privacy-Preserving Systems}\topicbadge{2.8}

	\textbf{Zcash and Financial Privacy:}
	Zcash uses zk-SNARKs to enable private transactions on a public blockchain.
	\begin{parts}
		\part[5] Explain what a shielded transaction proves without revealing its contents. Be specific about what remains hidden and what becomes public.
...
\begin{tcolorbox}[colframe=EarthBrown!30!white,colback=EarthBrown!5!white]
	\section*{Bonus Question\hfill\topicpill{2.8}}
	\begin{questions}
		\bonusquestion[10] You're designing a multi-signature wallet where any 2 out of 3 parties can authorize a transaction.
		\begin{parts}
