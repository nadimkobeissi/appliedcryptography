...
\begin{questions}
	\question[10] \textbf{Pseudorandom Generators}\topicbadge{1.4}
	...
	\begin{parts}
		...
		\end{align*}
		Determine whether $H$ is a secure PRG. If not, provide a distinguisher that can tell $H(S)$ apart from a truly random string with non-negligible advantage.
	\end{parts}

	\question[10] \textbf{Pseudorandom Functions and Permutations}\topicbadge{1.4}

	\begin{parts}
		\part[5] Consider the following PRF construction: $F(K, X) = G(K) \oplus X$, where $G$ is a secure PRG. Is $F$ a secure PRF? If not, describe a distinguisher that can tell $F$ apart from a random function with non-negligible advantage.
...
\begin{questions}
	\question[10] \textbf{CPA Security}\topicbadge{1.5}
	...
	\begin{parts}
		...
		\begin{subparts}
			\subpart $\textsf{Enc}(K, M) = (R, F(K, R) \oplus M)$ where $R \twoheadleftarrow \bits^{\lambda}$ and $F$ is a secure PRF.
			\subpart $\textsf{Enc}(K, M) = (R, F(K, M) \oplus R)$ where $R \twoheadleftarrow \bits^{\lambda}$ and $F$ is a secure PRF.
			\subpart AES in Electronic Codebook (ECB) mode.
			\subpart AES in Counter (CTR) mode with a randomly chosen IV.
		\end{subparts}
	\end{parts}

	\question[10] \textbf{CCA Security and Authenticated Encryption}\topicbadge{1.5}
	...
	\begin{parts}
		...
		\begin{subparts}
			...
			\subpart Calculate the approximate number of oracle queries needed to recover a 1KB file using the null-oracle attack, and explain why this is practical for an attacker.
		\end{subparts}
		...
		\begin{subparts}
			...
			\subpart AES-GCM is sometimes implemented with different tag lengths. Analyze the security trade-offs when using 128-bit tags versus 64-bit or 32-bit tags.
...
\begin{questions}
	\question[15] \textbf{Hash Function Properties}\topicbadge{1.6}
	...
	\begin{parts}
		...
		\begin{subparts}
			...
			\subpart Using the birthday paradox, calculate approximately how many hashes must be computed to find a collision with 50\% probability in a secure 256-bit hash function.
	...
	\question[15] \textbf{Password Hashing}\topicbadge{1.6}
	...
	\begin{parts}
		...
		\begin{subparts}
			\subpart Explain why memory-hard functions like Scrypt provide better protection against specialized hardware attacks than PBKDF2.
			\subpart Describe how each of Scrypt's parameters ($N$, $r$, $p$) affects its security and performance.
...
\begin{questions}
	\question[10] \textbf{Block Cipher Modes Analysis}\topicbadge{1.5}
	...
	\begin{parts}
		...
		\begin{itemize}
			...
			\item Discuss the performance trade-offs of AES-GCM compared to using separate encryption (CTR mode) and authentication (HMAC).
	...
	\question[10] \textbf{Hash Function Security Analysis}\topicbadge{1.6}
	...
	\question[10] \textbf{Password Management System Design}\topicbadge{1.6}
...
\begin{tcolorbox}[colframe=EarthBrown!30!white,colback=EarthBrown!5!white]
	\section*{Bonus Question\hfill\topicpill{1.4}}
	\begin{questions}
		\bonusquestion[10] The security of AES and other block ciphers depends on their resistance to various forms of cryptanalysis. Research and analyze one of the following advanced attacks:
