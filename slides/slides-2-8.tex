		...
		\begin{column}{0.5\textwidth}
			\begin{alertblock}{This is a proof technique}
				\begin{itemize}
					...
					\item Shows: if you can forge proofs, you must know the witness
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item ECDSA (related derivation from ElGamal)
			\end{itemize}
...
\begin{frame}{The attack: exploiting self-reference}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\end{itemize}
			\begin{alertblock}{The trick}
				Circuit outputs $(\gamma, \gamma-1)$ where $\gamma = \func{hash}{\langle C^* \rangle, y^*, \text{comm}(w)}$. Claim output is $(0,0)$ --- always false!
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item MLE of real output at $r$: $\hat{y}(r) = \gamma - r = 0$
...
\begin{frame}{Why this attack is devastating}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Attack needs: $d \geq d_{\text{comm}} + d_h + O(1)$
			...
			\begin{center}
				\resizebox{0.6\textwidth}{!}{
					\begin{msc}{}
						...
						\nextlevel[1]
						\action*{$\begin{array}{l}
									\text{result} := \text{apply}(\mathcal{B}, x, y) \\
