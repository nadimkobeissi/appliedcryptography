		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \textbf{Knowledge Soundness:}
				      \begin{itemize}
					      \item If prover succeeds, they \textit{know} a witness\footnote{A witness is the secret information or evidence that proves a statement is true - like the private key that corresponds to a public key, or the solution to a puzzle.}
				...
				\item \textbf{Essential when:}
				      \begin{itemize}
					      \item Building signature schemes
					      \item Proving knowledge of credentials
...
\begin{frame}{The power of Sigma protocols}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\textbf{Number Theory:}
			\begin{itemize}
				\item ``I know the square root of $X \bmod N$''
...
\begin{frame}{Example: extracting from Schnorr}
	\begin{center}
		\textbf{Given two accepting transcripts with the same $Y$ but different challenges:}
	\end{center}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Transcript 1:}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{enumerate}
				\item \textbf{Commit:}
				      \begin{itemize}
					      \item Pick $y \twoheadleftarrow \mathbb{Z}_n$
				...
				\begin{itemize}
					\item In Fiat-Shamir:
					      \begin{itemize}
						      ...
						      \item \textbf{Loses the deniability property}
					      \end{itemize}
				\end{itemize}
			\end{alertblock}
		\end{column}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item ECDSA (related, but not a direct Fiat-Shamir construction)
			\end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Basically impossible in practice!
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{As a circuit:}
...
\begin{frame}{What is a circuit digest?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item A circuit $C$ is a computation graph
				\item Gates, wires, input/output structure
				\item Can be serialized to bytes
				\item \textbf{Circuit digest:} $\psi = \langle C \rangle$
			\end{itemize}
			\textbf{The encoding process:}
			\begin{enumerate}
				\item Enumerate all gates in order
				\item Record gate types and connections
				\item Include input/output specifications
				\item Hash or encode to fixed-size digest
			\end{enumerate}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Why digests matter for the attack:}
			\begin{itemize}
				\item Circuit can ``know itself''
				\item Pass $\psi = \langle C^* \rangle$ as witness
				\item Circuit computes hash of own description
				\item Creates self-reference needed for attack
			\end{itemize}
			\begin{alertblock}{If a circuit is deep enough, it can:}
				\begin{itemize}
					\item Take its own digest as input
					\item Compute $\func{hash}{\psi, \ldots}$
...
\begin{frame}{Understanding the attack: a real-world analogy}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Imagine a magic lock:}
			\begin{itemize}
				\item The lock asks you a random question
				\item You must answer correctly to open it
				\item The question depends on what you show it
				\item Normally: you can't predict the question
			\end{itemize}
			\textbf{The honest scenario:}
			\begin{enumerate}
				\item Show the lock your credentials
				\item Lock generates random challenge
				\item You answer based on your secret
				\item Lock verifies and opens
			\end{enumerate}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{The attack:}
			\begin{itemize}
				\item You build a special mirror
				\item Mirror shows the lock... itself!
				\item Lock's question now depends on the mirror
				\item But mirror was built knowing the lock
				\item You pre-computed the ``random'' question!
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{The attack analogy continued: exam cheating}
	\begin{columns}[c]
		\begin{column}{0.67\textwidth}
			\textbf{Think of it like an exam:}
			\begin{itemize}
				\item Professor creates exam based on submitted essay
				\item Questions are ``random'' but derived from your essay
				\item Write essay first, then face unknown questions
			\end{itemize}
			\textbf{The cheater's trick:}
			\begin{enumerate}
				\item Submit an essay on ``how this exam will be graded''
				\item Essay contains the grading algorithm itself
				\item Essay predicts exactly which questions will be asked
				\item Cheater embeds the answers in the essay!
			\end{enumerate}
		\end{column}
		\begin{column}{0.33\textwidth}
			\textbf{Why it works:}
			\begin{itemize}
				\item ``Random'' challenge comes from hashing your input
				\item But your input describes the hash process
				\item You compute the hash inside your submission
				\item Your answer is tailored to that exact challenge
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{Why this attack is devastating}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{What makes this different:}
			\begin{itemize}
				\item GKR is widely used
				\item Works for \textit{any} hash
				\item Affects real deployed systems
			\end{itemize}
			\textbf{The depth requirement:}
			\begin{itemize}
				\item Attack needs: $d \geq d_{\text{comm}} + d_h + \mathcal{O}(1)$
				\item Circuit must be deep enough to compute hash + commitment
				\item Many practical circuits satisfy this!
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{alertblock}{The trade-off}
				Proving is 1,000--1,000,000$\times$ slower than native execution
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{ZK Battleship: proving without revealing}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Cheating is mathematically impossible
			\end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Proved ZK can work in an adversarial environment
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Code available on GitHub\footnote{\url{https://github.com/google/longfellow-zk}}
			\end{itemize}
			\textbf{Why this matters:}
			\begin{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Creditworthiness without credit score
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Prove you're human (not a bot)
