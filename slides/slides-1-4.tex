...
\begin{frame}{Enter pseudorandomness}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item We don't know how to make PRGs, or even whether they exist.
...
\begin{frame}{Attacking PRGs}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Assume that $G$ is a secure PRG.
...
\begin{frame}{Attacking PRGs}
	\begin{columns}[c]
		\begin{column}{0.45\textwidth}
			\begin{itemize}
				\item<1-2> Assume that $G$ is a secure PRG.
		...
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				...
				\item Basic operation:
				      \begin{itemize}
					      \item Initialize the S-box with a permutation of bytes 0--255.
					      \item Use the key to scramble the S-box.
...
\begin{frame}{PRG versus PRF: Key Differences}
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \textbf{PRG (Pseudorandom Generator)}:
				      \begin{itemize}
					      \item Takes a short seed, produces a longer output.
					      \item Generates the entire output as a monolithic string.
				...
				\item Taking $Y_1 \oplus Y_2$ causes $G(K)$ to cancel:
				      \begin{align*}
					      Y_1 \oplus Y_2 & = G(K) \oplus X_1 \oplus G(K) \oplus X_2 \\
					                     & = X_1 \oplus X_2
				      \end{align*}
				\item With a truly random function, $Y_1 \oplus Y_2 = X_1 \oplus X_2$ would be extremely unlikely!
			\end{itemize}
		\end{column}
		\begin{column}{0.4\textwidth}
			\begin{center}
...
\begin{frame}{PRF versus PRP}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Pseudorandom Function (HMAC-SHA-2)}
			\begin{itemize}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Versatility}: A secure PRP can be used as a PRF.
		      \begin{itemize}
			      ...
			      \item The reverse is not true (PRF $\rightarrow$ PRP conversion is complex).
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Pseudorandom permutation: definition}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item $n$ is the input length of the PRP. Also the output length!
				\item $\lambda$ is the key size and hence the security parameter.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item It doesn't scale, so we need practical approximate alternatives.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				...
				\item If each round function $F_i$ uses a distinct key $K_i$, this increases the security of the Feistel network against certain attacks.
	...
	\begin{columns}[c]
		\begin{column}{0.65\textwidth}
			\begin{itemize}
				...
				\item A meet-in-the-middle attack can break an $r$-round Feistel cipher with complexity $< 2^{|K|}$.
				\item The attack works by computing partial encryptions from both ends:
				      ...
				      \end{enumerate}
				\item With identical round functions, the effective security may be only \textbf{half} the number of rounds.
			\end{itemize}
		\end{column}
			...
			\begin{itemize}
				\item \textbf{Triple DES (3DES)} replaced it:
				      \begin{itemize}
					      ...
					      \item $C = E_{K_3}(D_{K_2}(E_{K_1}(P)))$
					      \item Effectively doubles the key length.
					      \item Compatible with legacy DES when $K_1 = K_2 = K_3$
				      \end{itemize}
				\item 3DES is still used in legacy systems, but has largely been replaced by AES for performance reasons.
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Despite these advances:
				      ...
				      \end{itemize}
				\item Even quantum computers offer only a modest advantage (Grover's algorithm reduces AES-128's security to $2^{64}$ operations).\footnote{More on quantum computers and how they affect cryptography later in the course.}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
