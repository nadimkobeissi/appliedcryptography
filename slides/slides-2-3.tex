...
\begin{frame}{Look at this mess}
	\bigimagewithcaption{pgp-keyserver.png}{A PGP ``key server''. The SKS keyserver network shut down in 2021 after ``certificate flooding'' attacks made it unusable.}
\end{frame}

\begin{frame}{Look at this mess}
	\bigimagewithcaption{pgp-pubkey.png}{Allegedly my public key. Is it even? I don't know!}
\end{frame}
...
\begin{frame}{Pidgin: an IM client with OTR support}
	\bigimagewithcaption{pidgin-chat.jpg}{Pidgin supported MSN Messenger, Yahoo Messenger, IRC, etc.}
\end{frame}

\begin{frame}{Pidgin: an IM client with OTR support}
	\bigimagewithcaption{pidgin-otr.jpg}{Pidgin was a popular way to use OTR.}
\end{frame}
...
\begin{frame}{OTR's legacy}
	\begin{itemize}
		...
		\item \textbf{Influenced modern messaging}
		      \begin{itemize}
			      \item The Signal Protocol builds directly on OTR's ideas
			      \item Its descendants now run on billions of devices: Signal, WhatsApp, Google Messages, Facebook Messenger
		      \end{itemize}
	\end{itemize}
\end{frame}
		...
		\begin{column}{0.4\textwidth}
			\begin{itemize}
				...
				\item \textbf{Forward Secrecy}
				      ...
				      \end{itemize}
				\item \textbf{Real world}: IKEv2, OTR, TLS 1.3
			\end{itemize}
		\end{column}
		\begin{column}{0.6\textwidth}
			\begin{center}
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Signatures resist KCI (as in SIGMA)
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{What goes wrong without proper key derivation?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\textbf{Why this matters:}
			\begin{itemize}
				\item Raw output is distinguishable from random
				\item AES's security proofs assume uniform keys
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item \textbf{Bit bias}: High bit always $0$ ($x < 2^{255} - 19$)
				\item \textbf{Small subgroups}: a few low-order points exist
				\item \textbf{Invalid points}: Half of $x$-values have no corresponding $y$
			\end{itemize}
			\begin{alertblock}{Real vulnerability}
				Attackers can force predictable shared secrets by sending low-order public keys
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Allegedly leaked by Chelsea Manning to WikiLeaks
...
\begin{frame}{Attacks on OTR version 2}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item \textbf{Attack scenario:}
				      \begin{itemize}
					      \item Attacker modifies messages to claim only OTRv1 is supported
				      \end{itemize}
				\item \textbf{The fix:}
				      \begin{itemize}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Mallory can convince Alice she completed an AKE with Bob
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item The deniability feature enables the attack!
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{X3DH: Signal's Asynchronous Key Exchange}
	\begin{itemize}
		...
		\item \textbf{2023 update}: Signal extended X3DH into \textbf{PQXDH}, mixing a post-quantum KEM (ML-KEM) into $S$ against ``harvest now, decrypt later''---more in topic 2.6
	\end{itemize}
\end{frame}
...
\begin{frame}{Telegram's MTProto}
	...
	\begin{itemize}
		...
		\item Multiple versions after vulnerabilities were found
	\end{itemize}
	\textbf{The Cryptographic Community Response}
	\begin{itemize}
		\item ``Don't roll your own crypto!''
		\item Multiple published attacks
		\item Academic papers showing weaknesses\footnote{\url{https://appliedcryptography.page/paper/\#telegram-exchange}}
	...
	\begin{itemize}
		...
		\item \textbf{Different threat model:}
		      \begin{itemize}
			      ...
			      \item Less focus on protecting from the service provider
		      \end{itemize}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Since late 2024: IP addresses and phone numbers handed to authorities upon valid legal requests
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}{How Signal handles groups today}
...
\begin{frame}{WhatsApp's approach: sender keys}
	\bigimagewithcaption{sender-keys.png}{Source: David Balbás, Daniel Collins, and Phillip Gajland, \textit{WhatsUpp with Sender Keys? Analysis, Improvements and Security Proofs}, IACR Asiacrypt, 2023.}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Encrypts to a public key, no interaction needed
				\item Used in MLS, TLS Encrypted Client Hello, and more
			\end{itemize}
...
\begin{frame}{TreeKEM}
	\bigimagewithcaption{treekem-a.png}{Source: Théophile Wallez}
\end{frame}

\begin{frame}{TreeKEM}
	\bigimagewithcaption{treekem-b.png}{Source: Théophile Wallez}
\end{frame}
