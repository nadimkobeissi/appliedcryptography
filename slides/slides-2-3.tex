...
\begin{frame}{How PGP works}
	...
	\begin{alertblock}{Hybrid Encryption}
		Actually uses symmetric crypto for the message, asymmetric for key exchange
	\end{alertblock}
\end{frame}
...
\begin{frame}{Look at this mess}
	\bigimagewithcaption{pgp-keyserver.png}{A PGP ``key server''}
\end{frame}
...
\begin{frame}{Because\ldots}{Identity substitution attack: Eve can replace Alice's identity}
	\vspace{-1cm}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{The Attack:}
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				\begin{tikzpicture}[>=Stealth, scale=0.9, transform shape]
					...
					\node[right, text width=2cm, align=left] at (8.5,-2.35) {\scriptsize\textcolor{red}{Eve intercepts}};
				\end{tikzpicture}
			\end{center}
			\textbf{Result:}
			\begin{itemize}
				\item Bob records $g^E$ as peer
				\item No cryptographic link between identity and key exchange
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Key Derivation Functions (KDFs)} solve this:
		      \begin{itemize}
			      ...
			      \item Examples: HKDF, NIST SP 800-56C, TLS-PRF
		      \end{itemize}
...
\begin{frame}{What goes wrong without proper key derivation?}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item \textbf{Bit bias}: Top 3 bits of the $x$-coordinate are never all 1
