		...
		\begin{column}{0.55\textwidth}
			\begin{itemize}
				\item Libraries are \textbf{interchangeable} when they:
				      ...
				      \end{itemize}
				\item i.e., when their usage and output are indistinguishable from the adversary's perspective when they are paired with \prog{}.
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item Even if encryption is randomized, decryption must always map each key-ciphertext pair back to the original message.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{itemize}
		...
		\item In Bitcoin's proof-of-work mechanism:
		      \begin{itemize}
			      ...
			      \item Current market cap for Bitcoin: approximately 1.7 trillion USD.
		      \end{itemize}
			...
			\begin{itemize}
				...
				\item For AES-256:
				      \begin{itemize}
					      ...
					      \item Far beyond the capability of any conceivable computation
				      \end{itemize}
		...
		\begin{column}{0.5\textwidth}
			\definitionbox{Negligible Functions}{
				...
				\begin{itemize}
					...
					\item For every polynomial $p$, we have $p(\lambda) \in \mathcal{O}(1/f(\lambda))$.
				\end{itemize}
			}
		\end{column}
		\begin{column}{0.5\textwidth}
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				...
				\item General question: If we take $q$ independent uniform samples from a set of $N$ items, what's the probability that some value is chosen more than once?
		...
		\begin{column}{0.4\textwidth}
			...
			\begin{itemize}
				...
				\item With $q \approx 1.2\sqrt{N}$, the probability is $\approx 0.5$
				\item For birthdays: we only need 23 people for a $>50\%$ chance
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{The ``bad event'' proof technique}{The principle}
	...
	\begin{itemize}[<+->]
		\item The left side is exactly the \textbf{advantage} from our indistinguishability definition: how well \prog{} tells the two libraries apart.
		\item The right side is the probability of \textbf{ever tripping the wire}.
		\item You can't distinguish the libraries any better than you can trigger the bad event.
...
\begin{frame}{The ``bad event'' proof technique}{Example: a warm-up claim}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{Claim:} these two libraries are \textbf{computationally indistinguishable}.
				\item \lib{}{1} returns \texttt{true} only when the caller's guess $X$ matches a fresh random string $R$.
				\item \lib{}{2} never returns \texttt{true} at all.
				\item Intuitively, guessing a uniformly random $\lambda$-bit string is \textbf{almost impossible} -- so \lib{}{1} practically never returns \texttt{true} either.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
	...
	\begin{columns}[c]
		\begin{column}{0.8\textwidth}
			\begin{itemize}[<+->]
				...
				\item Since advantage $\leq \prob{\texttt{bad}} \to 0$, we conclude \lib{}{1} $\approxeq$ \lib{}{2}. \textbf{Done!}
			\end{itemize}
		\end{column}
