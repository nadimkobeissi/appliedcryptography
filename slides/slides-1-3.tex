...
\begin{frame}{The ``bad event'' proof technique}{Definition}
	\definitionbox{Bad Event Technique}{
		...
		\\[1em]
		If \lib{}{1} and \lib{}{2} have identical source code, except for statements reachable only after setting \texttt{bad := true}, then:
		\begin{align*}
...
\begin{frame}{The ``bad event'' proof technique}{Key insights}
	\begin{itemize}
		...
		\item Practical application:
		      ...
		      \end{itemize}
		\item Enables us to focus on analyzing specific failure cases rather than the libraries' full behavior.
	\end{itemize}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item Yes! Let's prove it using the bad event technique.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Are these libraries \textbf{computationally indistinguishable?}
				\item Yes! Let's prove it using the bad event technique.
				\item Note: these libraries are interchangeable with \lib{}{1} and \lib{}{2}, respectively (as seen on the next slide).
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
	...
	\begin{columns}[c]
		\begin{column}{0.8\textwidth}
			\begin{itemize}
				...
				\item For any \prog{}, computationally speaking, we'd get essentially the same distribution of outputs.
			\end{itemize}
		\end{column}
