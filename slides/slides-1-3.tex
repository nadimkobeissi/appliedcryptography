...
\begin{frame}{Today's plan}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Provable security}
			\begin{itemize}
				\item Attack scenarios as \textbf{libraries}; adversaries as \textbf{programs} that call them.
				\item When are two libraries \textbf{interchangeable} ($\equiv$)?
				\item Attacks are \textbf{distinguisher programs}; proofs are chains of tiny, justified rewrites.
				\item Defining a primitive: syntax, correctness, security.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Computational cryptography}
			\begin{itemize}
				\item Why ``perfect'' is too strict: dismiss attacks that are too expensive or too lucky.
				\item Concrete vs.\ asymptotic security; polynomial time and negligible probability.
				\item Interchangeable ($\equiv$) becomes \textbf{indistinguishable} ($\approxeq$).
				\item Birthday probabilities and the \textbf{bad event} proof technique.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item Programs can only see the output of subroutine calls to libraries.
				      \begin{itemize}
...
\begin{frame}{When are libraries interchangeable?}
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			\begin{itemize}
				\item Libraries \lib{}{1} and \lib{}{2} are \textbf{interchangeable}, written \lib{}{1} \interchangeable{} \lib{}{2}, when they:
				      \begin{itemize}
					      \item Have the same interface,
					      \item Satisfy \prob{\prog{}\link\lib{}{1} \Rightarrow \texttt{true}} $=$ \prob{\prog{}\link\lib{}{2} \Rightarrow \texttt{true}} for \textbf{every} calling program \prog{}.
				      \end{itemize}
				\item i.e., no program whatsoever can tell them apart from the outputs it sees.
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item \textbf{Yes!} Their only difference is in \textit{unreachable lines of code}.
			\end{itemize}
		\end{column}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item $\|$ denotes string concatenation.
				      ...
				      \end{itemize}
				\item \textbf{Yes!} Outputting the concatenation of two uniformly sampled strings of lengths $n$ and $m$ is the same as outputting a single random string of length $n + m$.
			\end{itemize}
		\end{column}
		\begin{column}{0.05\textwidth}
		\end{column}
...
\begin{frame}{When are libraries interchangeable?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item Are these libraries interchangeable?
				\item \textbf{No!} The first library uses the same $K$ for every encryption, which breaks OTP security.
				      \begin{itemize}
					      \item Recall that OTP requires keys to be used only once (hence the name).
				      \end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.05\textwidth}
...
\begin{frame}{How do we show the distinguisher?}
	\begin{columns}[c]
		\begin{column}{0.4\textwidth}
			\begin{itemize}[<+->]
				\item What happens if we call $\func{otp.enc}{\bit{0}^n}$ multiple times?
				      \begin{itemize}
					      \item We'll get the same output, $K$, each time!
				      \end{itemize}
			...
			\begin{flushright}
				\sslinked{
					\ssprogram{}{
						...
						return $C_1 \oplus C_2 \stackrel{?}{=} M_1 \oplus M_2$
					}{0.85}
				}{\link}{
					\sslibrary{}{a}{
						$K \twoheadleftarrow \bits^n$\\[1em]
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item Are these libraries interchangeable?
				\item Let's find out!
				\item \textbf{Goal:} transform \lib{}{xor-samp-1} to \lib{}{xor-samp-2}, proving that each transformation step does not cause any change whatsoever to the library's behavior.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{flushright}
...
\begin{frame}{Proving two libraries are interchangeable}{Step 3}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item So, we can return $(X', Y)$ without any change to the library's behavior.
				      \begin{itemize}
					      \item $X' = Y \oplus M = (X \oplus M) \oplus M = X$.
				      \end{itemize}
			\end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item The intermediate libraries \lib{}{hyb-1}, \lib{}{hyb-2}, \ldots are called \textbf{hybrids}, and this is a \textbf{hybrid proof}. We will write many more of these.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{center}
...
\begin{frame}{Correctness of an SKE}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item Even so, decryption must \textbf{always} recover the original message: every key-ciphertext pair maps back to the plaintext it came from.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{One-time secrecy of an SKE}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item \ssprogram{}{
					      ...
					      return $C \stackrel{?}{=} \bit{0}^n$
				      }{1}
				\item \prob{\prog{}\link\lib{}{ots-real} \Rightarrow \texttt{true}} $= 1$
				\item \prob{\prog{}\link\lib{}{ots-rand} \Rightarrow \texttt{true}} $= \frac{1}{2^n}$
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Computational notions in cryptography}
	\begin{itemize}
		\item Security definitions in theoretical cryptography can be too strict:
		      \begin{itemize}
			      \item A scheme counts as ``broken'' even if the only attacks require a trillion years of computation.
			      \item Or even if attacks succeed with probability lower than that of winning the lottery 100 times in a row.
		      \end{itemize}
		\item Modern provable security takes a more practical approach:
		      ...
		      \end{itemize}
		\item Instead of ``no attack can succeed, not even in principle'', we prove: ``every attack has either ridiculously high cost or ridiculously small success probability''.
	\end{itemize}
\end{frame}
...
\begin{frame}{The concrete approach to provable security}
	\begin{itemize}
		...
		\item We rarely say definitively that a cryptographic algorithm ``is secure'' (though we did for OTP).
		\item Instead, we use statements like:
		      ...
		      \end{itemize}
		\item It is up to the user to judge whether this quantitative level of security is acceptable for their use case.
	\end{itemize}
\end{frame}

\begin{frame}{Monetary cost of huge computations}
	\begin{itemize}
		\item One way to grasp a huge computation is its price tag: roughly what $2^n$ CPU cycles would cost on the cheapest available Amazon EC2 instances (figures from \textit{The Joy of Cryptography}; orders of magnitude, not price quotes):
	\end{itemize}
	\begin{center}
		\small
		\begin{tabular}{cll}
			...
			$2^{65}$              & \$130,000                  & down payment on apartment                        \\
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\textit{\small Note: exponential growth eventually beats \textbf{every} polynomial.}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Understanding tiny probabilities}
	\begin{itemize}
		\item Let's put extremely small probabilities in perspective:
		      \begin{itemize}
			      \item In 2009, Patricia Demauro rolled dice 154 consecutive times in craps without getting a 7.
			      \item Probability of this event: $(30/36)^{154} \approx 2^{-40.5}$
		...
		\item Other extremely unlikely gambling events:
		      \begin{itemize}
			      ...
			      \item Probability: $(18/38)^{32} \approx 2^{-34.5}$
			      \item Winning the American Powerball lottery: $2^{-28.1}$
			      \item Winning Powerball twice in a row: $2^{-56.2}$
		      \end{itemize}
		\item So $2^{-40}$ is ``most improbable event in gambling history'' territory. The bounds we accept in cryptography ($2^{-64}$, $2^{-80}$, $2^{-128}$) are unimaginably rarer still.
	\end{itemize}
\end{frame}
...
\begin{frame}{Computational scale of Bitcoin mining}
	\begin{itemize}
		...
		\item These systems incentivize users to perform truly obscene amounts of computation: miners race to compute SHA-256 hashes as fast as possible.
		\item Order-of-magnitude figures, as of 2025:
		      \begin{itemize}
			      \item The network computes about $2^{70}$ SHA-256 hashes \textit{per second}, so roughly $2^{95}$ per year.
			      \item Over Bitcoin's entire history: roughly $2^{95}$--$2^{96}$ hashes in total.
			      \item Bitcoin's market capitalization: on the order of two trillion USD.
		      \end{itemize}
		\item This shows the enormous scale of computation that economic incentives can support\ldots
		\item \ldots and it is nowhere near $2^{128}$: at $2^{95}$ hashes per year, $2^{128}$ hashes would take $2^{33} \approx 8.6$ billion years.
	\end{itemize}
\end{frame}
...
\begin{frame}{AES: an example of asymptotic security analysis}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item The Advanced Encryption Standard (AES) is a common symmetric-key encryption algorithm. It comes in three variants: AES-128, AES-192, AES-256.
				\item In asymptotic language we'd say (a widely held \textit{conjecture}, not a theorem):
				      \begin{itemize}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Concrete security of AES-128: best known attack $\approx 2^{126.1}$ operations (biclique cryptanalysis, 2011) -- barely better than brute force ($2^{128}$), and hopelessly out of reach.
				\item AES-256: best known attack $\approx 2^{254.4}$ operations.
				\item Asymptotic approach: both are simply ``computationally secure''.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\definitionbox{Polynomial Running Time}{
				An algorithm runs in polynomial time if there is a polynomial $p$ such that the algorithm takes at most $p(n)$ steps on inputs of length $n$.
			}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\definitionbox{Negligible Functions}{
				A function $f$ is negligible if it approaches zero faster than $1/p(\lambda)$ for \textit{every} polynomial $p$. Formally, for every polynomial $p$:
				\begin{align*}
					\lim_{\lambda \to \infty} p(\lambda) \cdot f(\lambda) = 0.
				\end{align*}
				Equivalently: for every polynomial $p$, $f(\lambda) < 1/p(\lambda)$ for all sufficiently large $\lambda$.
			}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \textbf{You can probably ignore the definition and just think about this intuitively:}
				\item In cryptography, we want attack success probabilities to be negligible in the security parameter $\lambda$.
				\item Negligible probability: essentially zero for practical purposes when $\lambda$ is large enough.
				\item Examples: $2^{-\lambda}$ (guessing a $\lambda$-bit key) is negligible. $1/\lambda^{10}$ is \textbf{not}: multiply by $p(\lambda) = \lambda^{11}$ and it blows up.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\definitionbox{Indistinguishability}{
				...
				\end{align*}
				We write \lib{}{1} $\approxeq$ \lib{}{2} if every polynomial-time calling program has only negligible advantage in distinguishing them.
			}
		\end{column}
		\begin{column}{0.4\textwidth}
			\begin{itemize}
				...
				\item Again: this is intuitive, so no need to stress about the formal definition.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{The ``bad event'' proof technique}{Why we need a new tool}
	\begin{itemize}[<+->]
		\item So far, two libraries counted as \textbf{interchangeable} only when they behaved \textit{identically} -- differing at most in \textit{unreachable code} or an \textit{unused variable}.
		\item But very often two libraries are \textbf{almost} identical: they behave the same way \textit{except} in one rare situation, such as\ldots
		      \begin{itemize}
			      \item a collision between two random values,
			      \item correctly guessing a secret,
			      \item or sampling the same key twice.
		      \end{itemize}
		\item Intuitively they should \textit{still} be indistinguishable -- the ``bad'' situation almost never happens.
		\item The \textbf{bad event technique} is how we turn that intuition into an actual proof.
	\end{itemize}
\end{frame}

\begin{frame}{The ``bad event'' proof technique}{The big idea}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\definitionbox{The intuition}{
				Picture two libraries running the \textbf{same code}, side by side, in lockstep.
				\\[0.6em]
				They are wired so they can only ever \textbf{diverge} after crossing a single \hl{tripwire}.
				\\[0.6em]
				As long as nobody trips it, the two are \textbf{perfectly identical} -- no program can tell them apart.
				\\[0.6em]
				So the \textit{only} way to distinguish them is to \textbf{trip the wire}.
			}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item In code, we mark the tripwire by setting \hl{$\texttt{bad} \coloneq \texttt{true}$}.
				\item Once set, it stays \texttt{true} forever -- it's a tripwire, not a toggle.
				\item ``Triggering the bad event'' simply means \textbf{the code trips this wire}.
				\item Key consequence: a program's \textbf{advantage} can be no bigger than the \textbf{chance of tripping the wire}.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{The ``bad event'' proof technique}{The principle}
	\definitionbox{Bad Event Technique}{
		Suppose \lib{}{1} and \lib{}{2} run \textbf{identical code}, except for lines that can only be reached \textit{after} the flag has been set with \hl{$\texttt{bad} \coloneq \texttt{true}$}. Then for every calling program \prog{}:
		\begin{align*}
			\left| \prob{\prog{} \link \lib{}{1} \Rightarrow \texttt{true}} - \prob{\prog{} \link \lib{}{2} \Rightarrow \texttt{true}} \right| \;\leq\; \prob{\prog{} \link \lib{}{1} \text{ triggers } \texttt{bad}}
		\end{align*}
	}
	\begin{itemize}[<+->]
		\item The left side is exactly the \textbf{advantage} from our indistinguishability definition.
		\item The right side is the probability of \textbf{ever tripping the wire}: you can't distinguish the libraries any better than you can trigger the bad event.
		\item So the whole proof reduces to one task: \textbf{show the bad event happens with negligible probability.}
	\end{itemize}
\end{frame}

\begin{frame}{The ``bad event'' proof technique}{Example: a warm-up claim}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{Claim:} \lib{}{1} $\approxeq$ \lib{}{2} (computationally indistinguishable).
				\item \lib{}{1} returns \texttt{true} only when the guess $X$ matches a fresh random $R$; \lib{}{2} never does.
				\item Not interchangeable: calling $\func{predict}{\bit{0}^\lambda}$ once distinguishes with advantage exactly $2^{-\lambda} \neq 0$.
				\item But guessing a random $\lambda$-bit string is \textbf{almost impossible}, so that advantage is negligible. Let's prove it.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
...
\begin{frame}{The ``bad event'' proof technique}{Example: plant the tripwire}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item The two libraries can only ever disagree in \textbf{one rare moment}: when $R = X$.
				\item So we plant the tripwire \hl{$\texttt{bad} \coloneq \texttt{true}$} at exactly that moment in both.
				\item Now \lib{'}{1} and \lib{'}{2} are \textbf{line-for-line identical}, except for the single highlighted line that runs \textit{after} the tripwire.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
...
\begin{frame}{The ``bad event'' proof technique}{Example: how the pieces connect}
	\begin{itemize}
		\item \lib{}{1} \interchangeable{} \lib{'}{1}: the flag is just an unused variable, and on the line where $R = X$, returning \texttt{true} is the same as returning $R \stackrel{?}{=} X$.
		\item \lib{'}{2} \interchangeable{} \lib{}{2}: \lib{'}{2} returns \texttt{false} on every path, so it behaves exactly like \lib{}{2}.
	\end{itemize}
	\begin{center}
		\sslinked{
			\sslinked{
...
\begin{frame}{The ``bad event'' proof technique}{Example: bound the bad event}
	\begin{columns}[c]
		\begin{column}{0.68\textwidth}
			\begin{itemize}[<+->]
				\item All that's left is to bound how likely the tripwire is tripped.
				\item The bad event is exactly $R = X$, with $R \twoheadleftarrow \bits^\lambda$ fresh and uniform. For a single call, $\prob{\texttt{bad}} = \prob{R = X} = 1/2^\lambda$.
				\item Across $q$ calls, a union bound gives $\prob{\texttt{bad}} \leq q/2^\lambda$ -- still \textbf{negligible} for any polynomial $q$ (the same ``rare collision'' flavor as the birthday bound).
				\item Since advantage $\leq \prob{\texttt{bad}} \leq q/2^\lambda$, which is negligible, we conclude \lib{}{1} $\approxeq$ \lib{}{2}. \textbf{Done!}
			\end{itemize}
		\end{column}
...
\begin{frame}{The ``end-of-time'' strategy for bad events}
	\begin{itemize}[<+->]
		\item Sometimes the bad event is awkward to reason about \textit{mid-execution} -- especially when the adversary gets to choose values as it goes.
		\item \textbf{The trick:} don't check the tripwire along the way. Instead:
		      \begin{enumerate}
			      \item Run the library normally, just \textbf{recording} the values it samples and sees.
			      \item At the \textbf{very end}, look back over that record and check whether the bad event ever happened.
		      \end{enumerate}
		\item Why this helps:
		      \begin{itemize}
			      \item It \textbf{separates} the library's normal behavior from the bad-event bookkeeping.
			      \item With every value in hand at once, \prob{\texttt{bad}} is far easier to bound -- often with a single union or birthday bound.
		      \end{itemize}
		\item We'll lean on this trick in the next topic!
	\end{itemize}
\end{frame}
...
\begin{frame}{What to take away from today}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item A security definition is a pair of \textbf{libraries}; secure means no calling program can tell them apart.
				\item \lib{}{1} \interchangeable{} \lib{}{2}: \textit{identical} output probabilities for \textbf{every} program.
				\item An \textbf{attack} is a distinguisher program; a \textbf{proof} is a chain of hybrids, each step a small, checkable rewrite.
				\item A primitive needs a syntax, a correctness property and a security definition.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Perfect secrecy is too strict, so we restrict adversaries to \textbf{polynomial time} and \textbf{negligible} advantage: \lib{}{1} $\approxeq$ \lib{}{2}.
				\item \textbf{Birthday bound:} a repeat among $q$ samples from $N$ values is likely once $q \approx \sqrt{N}$.
				\item \textbf{Bad events:} advantage $\leq \prob{\texttt{bad}}$, so bound the bad event and you're done.
				\item Next time: \textbf{pseudorandom generators}, a short seed instead of a long key.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
