		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item $\|$ denotes string concatenation.
				      ...
				      \end{itemize}
				\item \textbf{Yes!} Outputting the concatenation of two randomly sampled uniform strings of lengths $n$ and $m$ is the same as outputting a single random string of length $n + m$.
			\end{itemize}
		\end{column}
		\begin{column}{0.05\textwidth}
		\end{column}
		...
		\begin{column}{0.8\textwidth}
			\begin{itemize}
				\item We need to analyze how likely it is that the bad event happens:
				      \begin{itemize}
					      \item The bad event occurs when $R \stackrel{?}{=} X$.
