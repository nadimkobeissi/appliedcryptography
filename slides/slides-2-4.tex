			...
			\begin{itemize}
				...
				\item Three common approaches:
				      \begin{itemize}
					      ...
					      \item \textbf{SRP (Secure Remote Password):} Password-authenticated key exchange
					      \item \textbf{Hardware-backed authentication:} Passkeys, Apple Secure Enclave, etc.
				      \end{itemize}
				\item Trade-offs:
				      \begin{itemize}
					      \item Dual-password: Poor usability, users often reuse passwords
...
\begin{frame}{Password recovery expectations vs reality}
	\begin{itemize}
		...
		\item \textbf{Common mitigation strategies:}
		      \begin{itemize}
			      \item \textbf{Recovery keys:} A long random string users must store
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item Covers most iCloud data categories:
				      ...
				      \end{itemize}
				\item \textbf{Recovery:} user-managed recovery key, or Apple escrow (the default)
...
\begin{frame}{Research directions in usable key management}
	\begin{itemize}
		...
		\item Open problems:
		      \begin{itemize}
			      \item Balancing security with user-friendliness
...
\begin{frame}{WebAuthn PRF: symmetric keys from passkeys}{Quick aside}
	\begin{itemize}
		\item WebAuthn's \textbf{PRF (Pseudorandom Function)} extension enables key derivation
		...
		\item Support has broadened since 2024 (Chrome, Safari 18+, platform authenticators), but remains uneven across hardware security keys
	\end{itemize}
\end{frame}
...
\begin{frame}{Using passkeys for key management}{Quick aside}
	\begin{itemize}
		...
		\item Current limitations:
		      \begin{itemize}
			      \item PRF support is now broad, but not yet universal
...
\begin{frame}{File sharing challenges}
	\begin{itemize}
		...
		\item Keys cannot be sent unprotected via an untrusted provider
...
\begin{frame}{MEGA: Where's the attack?}
	\bigimagewithcaption{mega-keys.png}{MEGA's key hierarchy. The master key encrypts the share, chat, sign,
		and node keys using AES-ECB. Source: \url{https://appliedcryptography.page/paper/\#mega-awry}}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Attack efficiency:}
				      \begin{itemize}
					      \item Binary search requires ${\sim}$512 login attempts\footnote{Later improved to 2 logins: \url{https://appliedcryptography.page/paper/\#mega-recovery}}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\begin{tikzpicture}[scale=0.8]
				...
				% Iteration count
				\node[draw, rectangle, fill=red!20] at (4.5,0) {512 iterations};
			\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{table}
				...
				\begin{tabular}{|l|c|c|l|}
					...
					\hline
					RSA Key Recovery   & 2 logins (was 512) & None                   & Share keys compromised \\
					\hline
			...
			\begin{itemize}
				...
				\item \textbf{Key rotation support:}
				      \begin{itemize}
					      ...
					      \item Uses the highest-indexed key for encryption
		...
		\begin{column}{1\textwidth}
			...
			\begin{enumerate}
				...
				\item \textbf{Ghost Key Attack}
				      \begin{itemize}
					      \item Forces the client to use an all-zero encryption key
					      \item Exploits missing bounds checking
				      \end{itemize}
				\item \textbf{nonce Reuse in AES-GCM}
				      \begin{itemize}
					      \item Same nonce used when files are modified
		...
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				...
				\item \textbf{Attack scenarios:}
				      \begin{itemize}
					      \item \textbf{Plaintext recovery:} If the attacker knows one version
			...
			\begin{itemize}
				\item \textbf{Initial patches:}
				      \begin{itemize}
					      ...
					      \item Generate new nonce on file modification
					      \item But the Key Insertion attack remained\ldots
				      \end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Key finding:} Widespread cryptographic failures across the ecosystem
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item \textbf{Protocol downgrade (Seafile)}
				      \begin{itemize}
					      \item Forces client to use weak password hash
					      \item 3 iterations of SHA-1 instead of PBKDF2
...
\begin{frame}{Beyond password hashing: Zero-knowledge protocols}
	\begin{itemize}
		\item Password hashing has fundamental limitations:
		      \begin{itemize}
			      \item Server must receive the password's hash to verify
			      \item Vulnerable to offline dictionary attacks (even though salts and memory-hard functions do help)
			      \item Trade-off between security and performance
		      \end{itemize}
		\item \textbf{Zero-knowledge password protocols} offer better security:
		      \begin{itemize}
			      \item Server never sees the password or a password equivalent
...
\begin{frame}{Forward secrecy in OPAQUE}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{What is forward secrecy for passwords?}
				      \begin{itemize}
					      \item If server is compromised \textit{after} authentication
					      \item Attacker cannot test password guesses offline
					      \item Past sessions remain secure even with server data
				      \end{itemize}
				\item \textbf{Contrast with traditional password hashing:}
				      \begin{itemize}
					      \item Hash database breach $\rightarrow$ offline cracking possible
					      \item OPAQUE breach $\rightarrow$ offline cracking possible, but no pre-computation advantage
					      \item Past session keys remain secure (AKE forward secrecy)
				      \end{itemize}
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}{OPAQUE's security guarantees}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item[\textcolor{green}{\mycheckmark}] No pre-computation advantage
				\item[\textcolor{green}{\mycheckmark}] Forward secrecy for session keys
			...
			\begin{itemize}
				...
				\item \textbf{Why this matters:}
				      \begin{itemize}
					      ...
					      \item Server could decrypt the backup key directly!
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{The fundamental trust problem:}
				      \begin{itemize}
					      \item WhatsApp claims to use HSMs, but users cannot verify this
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \textbf{Why not implemented?}
				      ...
				      \end{itemize}
				\item \textbf{The irony:} Meta already runs key transparency for WhatsApp identity keys (2023) --- just not for the Backup Key Vault
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \textbf{What is the 64-digit key, really?}
				      \begin{itemize}
					      \item The raw 256-bit backup key $K$, printed as 64 hexadecimal characters
					      \item No checksum, no error detection
					      \item Hard to transcribe and store correctly
				      \end{itemize}
				\item \textbf{We already have friendlier encodings:}
				      \begin{itemize}
					      \item BIP39 mnemonic phrases (12--24 words) with a built-in checksum
					      \item Proven by a decade of cryptocurrency wallets
				      \end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
...
\begin{frame}{Next: A formal treatment from the literature}
	\begin{center}
		...
		\vspace{0.2cm}
		{\footnotesize by Matilda Backendal, Hannah Davis, Felix Günther, Miro Haller, and Kenneth G. Paterson --- CRYPTO 2024}
	\end{center}
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Solution:} Oblivious Pseudorandom Function (OPRF), also used by OPAQUE
				      \begin{itemize}
