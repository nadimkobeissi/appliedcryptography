		...
		\begin{column}{1\textwidth}
			\begin{itemize}[<+->]
				...
				\item \textbf{Modern alternatives:}
				      \begin{itemize}
					      \item \textbf{BLAKE2} (2012): Faster than MD5\footnote{\url{https://www.blake2.net}}
					      \item \textbf{BLAKE3} (2020): Highly parallelizable, extremely fast\footnote{\url{https://github.com/BLAKE3-team/BLAKE3}}
					      \item \textbf{KangarooTwelve} (2016) and \textbf{TurboSHAKE} (2023): High-performance variants\footnote{\url{https://datatracker.ietf.org/doc/rfc9861/}}
				      \end{itemize}
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}{What about attacks on hash functions?}
	\begin{itemize}
		...
		\item Finally, we'll show additional attacks on hash functions and discuss their practical evolution.
	\end{itemize}
\end{frame}
...
\begin{frame}{Length extension attacks}{SHA-1 and SHA-2 vulnerability}
	\definitionbox{Length Extension Attack}{
		A \textbf{length extension attack} allows an attacker who knows $H(M)$ to calculate $H(M \| P \| X)$ for the padding $P$ and any attacker-chosen $X$, without knowing $M$ itself.
	}
	\begin{itemize}
...
\begin{frame}{HMAC: Protection against length extension}
	...
	\end{align*}
	\begin{itemize}
		\item $\mathsf{opad}$ and $\mathsf{ipad}$ are fixed padding values.
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item By predicting serial numbers and timestamps, they crafted a malicious certificate with the same MD5 hash as a legitimate one.
	...
	\begin{itemize}[<+->]
		...
		\item Simple approach: Iterate the hash function thousands of times.
		      \begin{itemize}
			      ...
			      \item \textbf{Example}: PBKDF2 (now considered obsolete).
		      \end{itemize}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.4\textwidth}
			\begin{tcolorbox}[colback=black!5!white,colframe=ciphergray]
				\small
				\begin{align*}
					U_1 & = \mathsf{PRF}(\mathsf{Password}, \mathsf{Salt} \| \mathsf{Int}(1)) \\
					U_2 & = \mathsf{PRF}(\mathsf{Password}, U_1)                              \\
					U_3 & = \mathsf{PRF}(\mathsf{Password}, U_2)                              \\
					\vdots                                                                    \\
					U_c & = \mathsf{PRF}(\mathsf{Password}, U_{c-1})                          \\
		...
		\begin{column}{0.6\textwidth}
			\begin{itemize}
				...
				\item \textbf{Solution}: Memory-hard functions
				      \begin{itemize}
					      \item Requires significant memory speed.
...
\begin{frame}{Blockchain and hash functions}
	\begin{itemize}
		\item Bitcoin and blockchain technology rely heavily on hash functions:
		      \begin{itemize}
			      \item \textbf{Proof-of-work}: Finding partial preimages (leading zeros).
		...
		\item The extreme computational cost of Bitcoin mining ($\approx$90 TWh/year) demonstrates hash function security.
...
\begin{frame}{Scrypt: A memory-hard password hashing function}
	\definitionbox{Scrypt}{
		\textbf{Scrypt} is a password-based key derivation function designed to be computationally and memory--speed-intensive, making it resistant to hardware acceleration attacks.
	}
	\begin{itemize}
			...
			\begin{itemize}
				...
				\item Memory hardness creates multiple benefits:
				      \begin{itemize}
					      ...
					      \item Memory is expensive and its speed scales poorly on specialized hardware.
					      \item Time-memory tradeoffs are possible but highly unfavorable.
					      \item Using less memory makes computation drastically slower.
				      \end{itemize}
...
\begin{frame}{Trend 2: Hardware diversity in attacks}
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			\begin{itemize}
				\item \textbf{Assimilation Bridge}: Integrates CPUs, FPGAs, and embedded devices.
	...
	\begin{columns}
		\begin{column}{\textwidth}
			\begin{itemize}
				...
				\item Consider modern memory-hard functions like Argon2 or Scrypt.
				      \begin{itemize}
					      \item Tune parameters based on your server capabilities.
					      \item Aim for 0.5--1 second processing time per hash.
				      \end{itemize}
				\item Keep your hashing mechanism current:\footnote{\url{https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html}}
				      \begin{itemize}
...
\begin{frame}{Hash functions can't be truly pseudorandom}
	\begin{itemize}
		\item No deterministic algorithm can produce truly random output.
