...
\begin{frame}{Why are these elements ``missing''?}
	\begin{itemize}[<+->]
		\item \textbf{The key requirement}: Elements in $\mathbb{Z}_n^*$ must be \textbf{coprime} to $n$
		      \begin{itemize}
			      \item Coprime means $\gcd(a, n) = 1$ (no common factors)
		...
		\item \textbf{Let's check the missing elements} (with $n = 15 = 3 \times 5$):
		      \begin{itemize}
			      \item $\gcd(3, 15) = 3$ (shares factor 3)
			      \item $\gcd(5, 15) = 5$ (shares factor 5)
			      \item $\gcd(6, 15) = 3$ (because $6 = 2 \times 3$)
			      \item $\gcd(9, 15) = 3$ (because $9 = 3^2$)
			      \item $\gcd(10, 15) = 5$ (because $10 = 2 \times 5$)
			      \item $\gcd(12, 15) = 3$ (because $12 = 4 \times 3$)
		      \end{itemize}
		\item \textbf{Why no inverse?} If $\gcd(a, n) > 1$, then $ax \equiv 1 \pmod{n}$ has no solution
		      \begin{itemize}
