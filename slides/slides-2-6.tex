...
\begin{frame}{Post-quantum TLS: the state of play}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{2024}: The tipping point---Chrome turns on hybrid PQ key agreement by default (April), with Cloudflare, Google, and AWS already supporting it server-side
...
\begin{frame}{Current deployment status (2026)}
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			...
			\end{itemize}
			\textbf{The result}: PQ now protects 72\% of all TLS connections seen by Cloudflare\footnote{\url{https://radar.cloudflare.com/post-quantum}}
		\end{column}
		\begin{column}{0.4\textwidth}
			\begin{center}
				\textbf{PQ Adoption (September 2026)}\\[3mm]
				\begin{tikzpicture}[scale=0.8]
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Previous attempts had security flaws}:
				      ...
				      \end{itemize}
				\item \textbf{Katana}: New RKEM based on Kyber\footnote{\url{https://appliedcryptography.page/paper/\#triple-ratchet}}
				      \begin{itemize}
