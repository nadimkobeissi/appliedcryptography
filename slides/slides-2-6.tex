	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Defense}: Double the key size\footnote{Cryptographers argue about this recommendation on various points, some thinking it's too simplistic, others thinking that with hash functions, you may not need to go from SHA-256 to SHA-512 with SHA-384 being enough, etc.}
				      \begin{itemize}
			...
			\begin{itemize}
				...
				\item \textbf{Symmetric cryptography}: Less impacted
				      \begin{itemize}
					      ...
					      \item Solution: Double the key size (AES-128 $\rightarrow$ AES-256)\footnote{Cryptographers argue about this recommendation on various points, some thinking it's too simplistic, others thinking that with hash functions, you may not need to go from SHA-256 to SHA-512 with SHA-384 being enough, etc.}
				      \end{itemize}
...
\begin{frame}{Why is this process efficient?}
	...
	{\scriptsize
		\begin{itemize}
			\item \textbf{Theoretical complexity}: $O(n^3)$ for $n \times n$ system
			      \begin{itemize}
				      {\scriptsize
					      \item Compare to brute force: $O(q^n)$ where $q$ is field size
...
\begin{frame}{Winternitz One-Time Signature (WOTS)}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item First hash the actual message to get a value in range $[0, w-1]$
			...
			\begin{itemize}
				\item 8-bit message: $M \in [0, 255]$
				      \begin{itemize}
					      ...
					      \item Manageable
				      \end{itemize}
				\item 32-bit message: $M \in [, 2^{32}-1]$
				      \begin{itemize}
					      \item Need up to 4 billion hashes!
					      \item Impractical
				      \end{itemize}
				\item 128-bit message: $M \in [, 2^{128}-1]$
				      \begin{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			...
			\begin{itemize}
				...
				\item In total, now 38\% of all TLS connections\footnote{\url{https://radar.cloudflare.com/adoption-and-usage\#post-quantum-encryption-adoption}}
			\end{itemize}
...
\begin{frame}{Understanding the communication cost table}
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			...
			\begin{itemize}
				\item Sends full Kyber-768 (2,272B) repeatedly
...
\begin{frame}{Three types of quantum attacks on blockchains}
	...
	\begin{itemize}
		\item \textbf{On-spend}: Derive private key while transaction sits in the mempool
		      \begin{itemize}
			      \item $\sim$9 min on fast-clock CRQC vs.\ Bitcoin's 10-min block time ($\sim$41\% success)
		      \end{itemize}
