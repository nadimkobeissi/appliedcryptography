...
\begin{frame}{How quantum computers work}
	\begin{itemize}
		...
		\item \textbf{Entanglement}: Qubits can be correlated in ways impossible classically
		      ...
		      \end{itemize}
		\item \textbf{Quantum parallelism}: Process all possibilities at once, extract the answer through interference
	\end{itemize}
\end{frame}
			...
			\begin{itemize}
				...
				\item \textbf{Symmetric cryptography}: Less impacted
				      \begin{itemize}
					      ...
					      \item Solution: Double the key size (AES-128 $\rightarrow$ AES-256)\footnote{Cryptographers argue about this recommendation on various points: some think it's too simplistic, others think that with hash functions you may not need to go all the way from SHA-256 to SHA-512, with SHA-384 being enough, etc.}
				      \end{itemize}
...
\begin{frame}{But resource estimates are shrinking fast}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{Google Quantum AI (2026)}\footnote{\url{https://appliedcryptography.page/paper/\#cryptocurrency-whitepaper}}: Compiled quantum circuits for Shor's algorithm targeting 256-bit ECDLP on secp256k1
			\end{itemize}
			\begin{center}
				\begin{tabular}{|l|c|c|}
					\hline
			...
			\end{center}
			\begin{itemize}
				\item \textbf{Physical cost}: $<$ 500,000 superconducting qubits (at $10^{-3}$ error rate)
				\item \textbf{Nearly 20$\times$ reduction} over prior estimates for a single ECDLP instance
...
\begin{frame}{Matrix form of linear equations}
	...
	\footnotesize
	{\scriptsize
		\textbf{Mathematical note:} The equation $\vec{b} = A\vec{s}$ represents a \emph{linear transformation} $T: \mathbb{Z}_q^5 \to \mathbb{Z}_q^5$ defined by $T(\vec{s}) = A\vec{s}$. Gaussian elimination $\rightarrow$ \emph{equivalent systems} via row operations that correspond to invertible elementary matrices.
	}
\end{frame}
...
\begin{frame}{Why is this process efficient?}
	Gaussian elimination achieves polynomial-time complexity:
	{\scriptsize
		\begin{itemize}
			\item \textbf{Theoretical complexity}: $\mathcal{O}(n^3)$ for $n \times n$ system
			      \begin{itemize}
				      {\scriptsize
					      \item Compare to brute force: $\mathcal{O}(q^n)$ where $q$ is field size
					      \item For $n=1000$: $\sim 10^9$ operations vs $q^{1000}$ (astronomical!)}
			      \end{itemize}

			\item \textbf{Algorithmic refinements} for practical speed:
			      \begin{itemize}
				      {\scriptsize
					      \item \textbf{Pivoting strategies}: Choose best pivot to minimize numerical errors
					      \item \textbf{Sparse methods}: Exploit matrices with mostly zeros
					      \item \textbf{Block algorithms}: Optimize for memory cache hierarchies}
			      \end{itemize}

			\item \textbf{Hardware acceleration} (critical for modern implementations):
			      \begin{itemize}
				      {\scriptsize
					      \item \textbf{GPU parallelism}: Thousands of cores compute simultaneously
					      \item \textbf{SIMD instructions}: Process multiple data elements per CPU cycle
					      \item \textbf{Optimized libraries}: BLAS, LAPACK, cuBLAS exploit hardware
					      \item \textbf{Real speedup}: 50--100$\times$ faster with good implementation}
			      \end{itemize}
			      {\scriptsize
			\item \textbf{Works in finite fields}: Same techniques apply modulo $q$}
		\end{itemize}}

	\vspace{2mm}
	{\scriptsize \textbf{Bottom line:} Modern systems solve $n \sim 10^6$ variables in seconds!}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.4\textwidth}
			...
			\begin{itemize}
				...
				\item Computes $\vec{x} = M\vec{s} + \vec{e}$
				\item Gives you $(M, \vec{x})$
			\end{itemize}
			\textbf{Scenario 2: Random}
			\begin{itemize}
				\item Alice picks random $\vec{x}$
				\item Gives you $(M, \vec{x})$
			\end{itemize}
		...
		\begin{column}{0.6\textwidth}
			...
			\begin{itemize}
				\item Noise $\vec{e}$ makes $\vec{x}$ look random
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item[$\times$] McEliece not selected by NIST (though HQC, also code-based, was in 2025)
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\begin{tikzpicture}[scale=0.7]
				...
				\node[draw,rectangle] (pk2) at (1.5,0) {$PK_2$};
				\node[draw,rectangle] (pk3) at (2.5,0) {$PK_3$};
				\node[draw,rectangle] (pk4) at (4,0) {$PK_4$};
				\node[draw,rectangle] (pk5) at (5,0) {$PK_5$};
				\node[draw,rectangle] (pk6) at (6.5,0) {$PK_6$};
				\node[draw,rectangle] (pk7) at (7.5,0) {$PK_7$};
				\node[draw,rectangle] (pk8) at (8,0) {$PK_8$};
...
\begin{frame}{NIST PQ standardization: The winners}
	\begin{itemize}
		\item \textbf{August 2024}: NIST released final standards after an 8-year process
...
\begin{frame}{TLS migration timeline}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{February 2024}: PQ TLS 1.3 adoption reaches critical mass\footnote{Many of the slides in this subsection are based on Bas Westerbaan's excellent blog post over at Cloudflare, but have been updated to account for developments in 2024 and 2025: \url{https://blog.cloudflare.com/pq-2024/}}
...
\begin{frame}{Cryptocurrency's unique quantum vulnerability}
	\begin{itemize}
		...
		\item \textbf{Unlike TLS}: Blockchain keys are often exposed \textit{permanently} on-chain
		      ...
		      \end{itemize}
		\item \textbf{Total}: $\sim$6.7 million BTC in quantum-vulnerable addresses\footnote{\url{https://quantumai.google/static/site-assets/downloads/cryptocurrency-whitepaper.pdf}}
	\end{itemize}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Example}: McEliece (hardness of decoding a linear code with insufficient information)
				      \begin{itemize}
					      \item In the worst case, some argue it could be proven NP-hard
					      \item Sounds amazing: NP-hard = practically unbreakable!
					      \item But cryptographic security is based on the average case, not the worst case\ldots
					      \item And even here, the claim of NP-hardness is debated\ldots\footnote{Eurocrypt 2025 paper found a subexponential distinguisher for McEliece: \url{https://appliedcryptography.page/paper/\#syzygy-distinguisher}}
				      \end{itemize}
