...
\begin{frame}{Physical challenges: Why qubits are so difficult}
	\begin{itemize}
		\item \textbf{Size requirements}: Qubits are difficult to physically manipulate
		      ...
		      \end{itemize}
		\item \textbf{Temperature requirements}: $\approx$10--20 millikelvin --- hundreds of times colder than outer space
		      \begin{itemize}
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Defense}: Double the key size\footnote{Cryptographers argue about this recommendation on various points: some think it's too simplistic, others think that with hash functions you may not need to go all the way from SHA-256 to SHA-512, with SHA-384 being enough, etc.}
				      \begin{itemize}
...
\begin{frame}{But resource estimates are shrinking fast}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			...
			\end{center}
			\begin{itemize}
				\item \textbf{Physical cost}: $<$500,000 qubits at $10^{-3}$ error rate---nearly 20$\times$ below prior estimates
				\item Same story for RSA: 2025 estimates cut RSA-2048 to $<$1M noisy qubits (20$\times$ below 2019)
...
\begin{frame}{What is Gaussian elimination?}
	\begin{itemize}
		...
		\item \textbf{Basic idea}: Transform into an equivalent, simpler form
		\item \textbf{Method}: Use row operations to eliminate variables step by step
		\item \textbf{Key insight}: Each row operation creates an \emph{equivalent system}
		      \begin{itemize}
			      \item Equivalent $\equiv$ different equations, same solutions
			      \item Each operation corresponds to multiplication by an elementary matrix
			      \item Result: Same solution set, easier to solve
		      \end{itemize}
		\item \textbf{Notation}:
		      \begin{itemize}
			      \item $R_1, R_2, R_3, \ldots$ = Row 1, Row 2, Row 3, etc.
		      \end{itemize}
		\item \textbf{Augmented matrix} $[A|\vec{b}]$: coefficient matrix with constants attached
	\end{itemize}
\end{frame}
...
\begin{frame}{Step 1: Set up the augmented matrix}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			Create augmented matrix $[A|\vec{b}]$:
			\[
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Modulus $q$ (e.g., 3329)\footnote{3329 is congruent to 1 modulo 256, and therefore the polynomial $X^{256} + 1$ splits into a product of degree $2$ polynomials of the form $(X^2 - r_{i})$.}\footnote{\url{https://appliedcryptography.page/paper/\#basic-lattice}}
	...
	\begin{itemize}
		...
		\item \textbf{Example} (simplified):
		      \begin{itemize}
			      ...
			      \item Kyber instead chooses parameters so that decryption failures are astronomically rare
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{The McEliece cryptosystem (1978)}
	\begin{itemize}
		...
		\item \textbf{Public key}: Generator matrix $G = S \cdot G' \cdot P$ (looks random)
		      \begin{itemize}
			      \item $S, G', P$ are special matrices (kept secret)
		...
		\item \textbf{Encryption}:
		      ...
		      \end{enumerate}
		\item \textbf{Decryption}: Only possible if you know $S, G', P$
		      \begin{itemize}
...
\begin{frame}{McEliece encryption: The basic setup}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\textbf{Bob creates his keys:}
			\begin{enumerate}
				\item Pick a ``nice'' error-correcting code
				      \begin{itemize}
					      \item Has special structure (secret)
					      \item Can efficiently correct $t$ errors
				...
				\item Disguise it with random matrices
				      ...
				      \end{itemize}
				\item Public key: $G$ (looks random!)
				\item Private key: $(S, G', P)$
			\end{enumerate}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{McEliece decryption: How Bob recovers the message}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\textbf{Bob receives $c = c_0 + e$ from Alice:}
			\begin{enumerate}
				\item Undo the permutation using \textcolor{blue}{his private key $P$}
				      \begin{itemize}
					      \item Compute: $c' = c \cdot P^{-1}$
					      \item This gives: $c' = m \cdot S \cdot G' + e'$
				      \end{itemize}
				\item Use special structure of \textcolor{blue}{his private key $G'$}
				      \begin{itemize}
					      \item Error-correct $c'$ using $G'$
					      \item Removes the $t$ errors
					      \item Gets: $m \cdot S \cdot G'$
				      \end{itemize}
				\item Recover original message using \textcolor{blue}{his private key $S$}
				      \begin{itemize}
					      \item Apply $S^{-1}$ to get $m$
				      \end{itemize}
			\end{enumerate}
		\end{column}
	\end{columns}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item[$\times$] Complex/costly key generation
	...
	\begin{itemize}
		\item \textbf{Efficiency with lattices/codes}:
		      \begin{itemize}
			      ...
			      \item Potentially fewer round trips
		      \end{itemize}
		\item \textbf{Mathematical limitations}:
		      \begin{itemize}
			      ...
			      \item Isogenies do support DH-style key agreement, but have other issues
		      \end{itemize}
...
\begin{frame}{X-Wing Design}
	\bigimagewithcaption{xwing-design.png}{Source: \url{https://appliedcryptography.page/paper/\#xwing-hybrid}}
\end{frame}
...
\begin{frame}{ML-KEM: The NIST standard}
	\begin{itemize}
		...
		\item \textbf{Three security levels}:
		      ...
		      \end{itemize}
		\item \textbf{First NIST PQ key encapsulation standard}
	\end{itemize}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			...
			\begin{itemize}
				...
				\item In total, PQ is now used in 38\% of all TLS connections\footnote{\url{https://radar.cloudflare.com/adoption-and-usage\#post-quantum-encryption-adoption}}
			\end{itemize}
			\textbf{Other browsers:}
			\begin{itemize}
				\item Firefox: since late 2024
...
\begin{frame}{Apple iMessage PQ3: High-level overview}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Device-to-device messaging:}\footnote{\url{https://appliedcryptography.page/paper/\#pq3-analysis}}
			\begin{itemize}
				\item Asynchronous communication
				\item Either party can send anytime
				\item Works when peer is offline
			\end{itemize}

			\textbf{Key components:}
			\begin{itemize}
				\item Apple IDentity Service (IDS)
				\item Long-term identity keys
				\item Pre-key material
				\item Double ratchet + PQ
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Session establishment:}
			\begin{enumerate}
				\item Query IDS for peer's pre-keys
				\item Derive initial keys
				\item Send first encrypted message
				\item Peer verifies and derives keys
				\item Session established
			\end{enumerate}

			\textbf{Two ratchet types:}
			\begin{itemize}
				\item \textbf{Symmetric}: Same direction
				\item \textbf{Public-key}: Direction change
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{PQ3: Security guarantees}
	\begin{itemize}
		\item \textbf{Hybrid construction benefits:}
		      \begin{itemize}
			      \item Secure if EITHER classical OR PQ remains unbroken
			      \item At minimum: Classical security level
			      \item Plus: Protection against quantum adversaries
		      \end{itemize}
		\item \textbf{Post-compromise security:}
		      \begin{itemize}
			      \item Regular KEM ratcheting provides PQ healing
			      \item Even if KEM shared secret compromised
			      \item Next KEM refresh restores security
			      \item Works against ``harvest now, decrypt later''
		      \end{itemize}
		\item \textbf{Forward secrecy:}
		      \begin{itemize}
			      \item Per-message keys from chain ratchet
			      \item Delete old keys immediately
			      \item Past messages safe even if current key leaked
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Understanding the communication cost table}
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			\textbf{Key parameter: p}
			\begin{itemize}
				\item Probability sender continues without receiving reply
				\item p = 0: Perfectly balanced (sender waits for reply)
				\item p = 0.5: Realistic (typing indicators, read receipts)
				\item p = 0.9: Offline device (laptop turned off)
			\end{itemize}

			\textbf{Why PQ3 costs explode:}
			\begin{itemize}
				\item Sends full Kyber-768 (2,272 B) repeatedly
				\item Until acknowledgment received
				\item High p = many retransmissions
			\end{itemize}
		\end{column}
		\begin{column}{0.45\textwidth}
			\textbf{Why TR stays efficient:}
			\begin{itemize}
				\item Erasure codes spread data across messages
				\item Each message carries one chunk
				\item No repeated full KEM messages
				\item Works even with offline devices
			\end{itemize}

			\textbf{Real-world impact:}
			\begin{itemize}
				\item Typing indicators common
				\item Linked devices often offline
				\item p = 0.9 realistic for Signal
				\item TR: 5$\times$ better than PQ3
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Full KEM every $\approx$50 messages
...
\begin{frame}{OpenSSH's aggressive stance}
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			...
			\begin{quote}
				\small
				\texttt{** WARNING: connection is not using a post-quantum key exchange algorithm.\\
					** This session may be vulnerable to "store now, decrypt later" attacks.}
			\end{quote}

			\textbf{Why this matters:}
			\begin{itemize}
				\item Pushes ecosystem forward
				\item Makes PQ the norm, not exception
			\end{itemize}
		\end{column}
		\begin{column}{0.45\textwidth}
			...
			\begin{itemize}
				...
				\item Long-lived connections
				\item High-value targets (admin access)
				\item Perfect for SNDL attacks
			\end{itemize}

			\textbf{Deployment advantage:}
			\begin{itemize}
				\item Simpler than TLS
				\item No certificate chains
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Ethereum: EIP-7932 proposes PQ precompiles
				\item Margin for error ``increasingly narrow''
			\end{itemize}
			\begin{quote}
				\small ``It is conceivable that the existence of early CRQCs may first be detected on the blockchain rather than announced.''
			\end{quote}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.55\textwidth}
			\begin{itemize}
				...
				\item \textbf{Google's approach}\footnote{\url{https://research.google/blog/safeguarding-cryptocurrency-by-disclosing-quantum-vulnerabilities-responsibly/}}: Use a ZKP to prove the resource estimates
...
\begin{frame}{Side-channel attacks: The hidden threat}
	\begin{itemize}
		...
		\item \textbf{Why PQ crypto is vulnerable}: Complex mathematical operations
		      \begin{itemize}
			      \item Polynomial and matrix arithmetic, rejection sampling (lattice-based)
