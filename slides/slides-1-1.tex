			...
			\begin{itemize}
				\item \textbf{Initial key exchange}: Uses X3DH (Extended Triple DH)
				      \begin{itemize}
					      ...
					      \item Works even when the recipient is offline (\textit{``asynchronous''} protocol).\footnote{Everything on this slide will be covered in much more detail later in the course.}
				      \end{itemize}
...
\begin{frame}{TLS 1.3: high-level sketch}
	\bigimagewithcaption{tls-13-sketch}{Source: Mostafa Ibrahim}
\end{frame}
...
\begin{frame}{How TLS 1.3 was made}
	\bigimagewithcaption{fischer-tls-13-bubbles}{}
\end{frame}
...
\begin{frame}{Exam logistics}
	\begin{columns}[c]
		\begin{column}{0.65\textwidth}
			\begin{itemize}[<+->]
				\item Will be held at the Lebanese University campus in Hadath, Beirut.
				\item High achievers will receive a recommendation letter from me + a certificate.
				\item Certificate can be authenticated online (to verify it's genuine).
				\item Encourages you to learn + gives you a credential for your CV and grad studies!
			\end{itemize}
		\end{column}
		\begin{column}{0.35\textwidth}
			\imagewithcaption{website-midterm.png}{Check the website for complete exam information! \\ \url{https://appliedcryptography.page}}
