	...
	\begin{itemize}[<+->]
		...
		\item \textbf{What we are actually here to learn:}
		      \begin{itemize}
			      ...
			      \item How a primitive's \emph{interface}, not just its internals, can sink every \emph{protocol} built on it.
			...
			\begin{itemize}[<+->]
				...
				\item \textbf{September 1994:} the algorithm is posted anonymously to the Cypherpunks mailing list.
				      \begin{itemize}
					      ...
					      \item RSA never confirmed it, so the public version was politely called \textbf{``Alleged RC4''} or \textbf{ARCFOUR} --- a name OpenSSH kept until 2017 and GnuTLS keeps today.
				      \end{itemize}
			\end{itemize}
		\end{column}
...
\begin{frame}{Early warnings (1995--2001)}
	...
	\end{itemize}
	\onslide<+->{\centering\textit{Every one of these was public. Every one was deployed straight past.}}
\end{frame}
		...
		\begin{column}{0.48\textwidth}
			\onslide<+->{
				\definitionbox{Why this is devastating}{
					Take one plaintext byte encrypted under many independent keys --- say, a byte of a session cookie, resent at the same position in every request.
					\vspace{0.5em}
					\\
...
\begin{frame}{One-time secrecy of an SKE}{Reminder}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\\[1em]
			\textbf{Mantin and Shamir just handed us a distinguisher between these two libraries:} call \textsf{ots.enc} on a fixed $M$ a few thousand times, and count how often byte 2 of $C$ equals $M_2$.
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{In fact, byte 2 is only the beginning}
	\begin{center}
		\vspace{-0.3em}
		\begin{tikzpicture}
			\node[fill=white, anchor=center, inner sep=1mm]{
				\includegraphics[height=0.65\textheight, keepaspectratio]{images/rc4-bias.png}
			};
		...
		\begin{column}{0.44\textwidth}
			\begin{itemize}
				\item \textbf{A 104-bit key $K$}, shared by every device on the network.
				      \begin{itemize}
					      \item Typed in directly, \emph{no} KDF. Originally 40 bits, for export.
				      \end{itemize}
				\item \textbf{A 24-bit IV}, chosen per packet.
				\item \textbf{Per packet:} append a \textbf{CRC-32} checksum (the ICV), encrypt both with RC4 under $IV \,\|\, K$, and send the IV \textbf{in the clear}.
			\end{itemize}
			\begin{center}
	...
	\begin{columns}[c]
		\begin{column}{0.54\textwidth}
			\begin{itemize}[<+->]
				...
				\item To attack secret byte $K_{A+1}$, which sits at position $A+3$ of the RC4 key, wait for an IV of the form $(A+3,\ 255,\ X)$, where $X$ is anything.
...
\begin{frame}{Why $(A+3, 255, X)$? Let us trace it}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item $Z_1$ \textbf{is} whatever step $i=3$ deposited at $S[3]$: the entry at index $X + 6 + K_1$ of the state $S$ just before that step, which depends only on the IV. Rebuild $S$ and invert it: $K_1 = S^{-1}[Z_1] - X - 6$.
...
\begin{frame}{From paper to practice, in weeks}
	\begin{itemize}[<+->]
	...
	\end{itemize}
	\onslide<+->{\centering\textbf{From ``interesting theoretical result'' to script-kiddie tool: a few weeks.}}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.54\textwidth}
			\begin{itemize}[<+->]
				...
				\item \textbf{PTW --- Tews, Weinmann and Pyshkin (2007):} combine Klein's correlation with key ranking, and recover each key byte independently instead of left to right, so one wrong byte no longer poisons the rest.
				      \begin{itemize}
		...
		\begin{column}{0.46\textwidth}
			\onslide<+->{
				\definitionbox{The trend line is the lesson}{
					...
					\\[0.5em]
					Between 2001 and 2007 the weakness stayed the same --- the key schedule leaking into the first bytes. Only the exploitation improved.
				}
			}
...
\begin{frame}{What WEP got wrong}
	\begin{itemize}[<+->]
		\item \textbf{It fed attacker-visible data straight into the key.} Concatenating a public IV with a secret key is not key derivation. WPA's TKIP fixed it with a per-packet \textbf{key-mixing function}; WPA2 gave AES a proper nonce instead.
	...
	\end{itemize}
	\onslide<+->{\centering\textbf{Only the first needed RC4.} The other three would have sunk WEP with any cipher.}
\end{frame}
...
\begin{frame}{AlFardan et al. (2013): the biases are real, and reachable}
	\begin{columns}[c]
		\begin{column}{0.52\textwidth}
			\begin{itemize}[<+->]
				\item Rather than reason analytically, five researchers \emph{measured} RC4's keystream over $2^{44}$ random keys.\footnote{\url{https://appliedcryptography.page/paper/\#rc4-tls}} The bias is not confined to byte 2, and needs no weak key: \textbf{every one} of the first 256 positions is non-uniform.
				\item $2^{26}$ sessions recovered the first $40$ bytes of application data at over $50\%$ per byte; $2^{32}$ got over $96\%$ for all $220$ within reach (keystream bytes $1$--$36$ encrypt the handshake's \textsf{Finished} message).
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
			\onslide<+->{
		...
		\begin{column}{0.46\textwidth}
			\onslide<+->{
				\definitionbox{Why this one matters most}{
					...
					\vspace{0.4em}
					\\
					It also relates a byte to \emph{another byte of the same plaintext} --- exactly what you want against structured plaintext like a password.
				}
			}
		\end{column}
	\end{columns}
...
\begin{frame}{2015: three attacks finish the job}
	\begin{itemize}[<+->]
		\item \textbf{``Bar Mitzvah'' (Mantin, Black Hat Asia).} Revisits the \textbf{invariance weakness} from the 2001 FMS paper: for a class of weak keys, structure in the key survives the KSA and leaks plaintext. The name: a weakness finally coming of age at thirteen.
		\item \textbf{Garman, Paterson and van der Merwe (USENIX Security),} \textit{Attacks Only Get Better}.\footnote{\url{https://appliedcryptography.page/paper/\#rc4-attacks}}
		      \begin{itemize}
			      ...
			      \item Combine the keystream biases with how humans actually choose passwords: Bayesian inference over a dictionary.
		      \end{itemize}
		...
		\begin{column}{0.55\textwidth}
			\begin{itemize}[<+->]
				...
				\item A password is not random at all:
				      \begin{itemize}
					      \item Drawn from a small, skewed alphabet, then base64-encoded in Basic Auth, which imposes a rigid known format.
					      \item Very likely to be in a leaked password list.
				      \end{itemize}
				\item So the attacker never has to \emph{recover} the password --- only narrow the candidates until a dictionary finishes: $2^{26}$ encryptions, against $\approx 2^{34}$ for a cookie.
			\end{itemize}
		\end{column}
		\begin{column}{0.45\textwidth}
			\onslide<+->{
				\alertbox{The general principle}{
					Statistics and structure \textbf{multiply}. Every byte of context you already know is a byte the statistics no longer has to earn.
					\vspace{0.4em}
					\\
...
\begin{frame}{Six lessons}
	\begin{itemize}[<+->]
		...
		\item \textbf{3. ``Theoretical'' means ``practical, later.''} FMS to AirSnort took a few weeks. Mantin--Shamir to a TLS break took twelve years, but it arrived.
...
\begin{frame}{What replaced RC4, and why it fails differently}
	\begin{columns}[c]
		\begin{column}{0.54\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{It takes a nonce.} ChaCha20 and AES-GCM both do, so WEP's improvised per-message keys are not even expressible.
				\item \textbf{It has no evolving secret state.} Each block is an independent function of $(K, N, \text{ctr})$: nothing long-lived to leak, no key-schedule residue in the output.
				\item \textbf{It comes bolted to a MAC.} \textbf{ChaCha20-Poly1305} is AEAD, so WEP's forgeable CRC-32 could not happen; TLS 1.3 allows nothing else.
			\end{itemize}
		\end{column}
		\begin{column}{0.46\textwidth}
			\onslide<+->{
				\definitionbox{The structural difference}{
					...
					\vspace{0.4em}
					\\
					The second is a PRF evaluated at distinct points --- a shape we know how to reason about, shared with AES-CTR.
				}
			}
			\onslide<+->{\centering\textbf{It fails differently:} repeat a nonce under one key and you are back to a two-time pad.}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{0.72\textwidth}
			\begin{enumerate}
				...
				\item \textbf{What are we doing right now that will look like this in 2040?}
				      \begin{itemize}
					      \item Candidates: RSA-2048? SHA-1 in Git? 3DES in payment terminals?
				      \end{itemize}
			\end{enumerate}
		\end{column}
