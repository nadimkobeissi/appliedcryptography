...
\begin{frame}{TLS attacks timeline (to 2015)}
	\bigimagewithcaption{tls-attacks-timeline-2015.png}{TLS attacks up until 2015.}
\end{frame}
...
\begin{frame}{Notice how we're mentioning design requirements now?}
	\begin{center}
		\bigimagewithcaption{fischer-sectioned.png}{Fischer et al., The Challenges of Bringing Cryptography from Research Papers to Products: Results from an Interview Study with Experts, USENIX Security 2024}
	\end{center}
\end{frame}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Four components of a cipher suite}:
		      \begin{enumerate}
			      ...
			      \item \textbf{MAC algorithm}: How to ensure integrity (SHA256, SHA384)
		      \end{enumerate}
...
\begin{frame}{Let's Encrypt: active certificates}
	\bigimagewithcaption{letsencrypt-certs.png}{Source: Let's Encrypt}
\end{frame}

\begin{frame}{Let's Encrypt: certificates issued}
	\bigimagewithcaption{letsencrypt-issuance.png}{Source: Let's Encrypt}
\end{frame}

\begin{frame}{Percentage of HTTPS traffic in Firefox}
	\bigimagewithcaption{letsencrypt-https.png}{Source: Let's Encrypt}
\end{frame}
...
\begin{frame}{TLS: the handshake protocol}
	\bigimagewithcaption{tls-13-handshake}{TLS 1.3 handshake protocol. Source: Serious Cryptography}
\end{frame}
...
\begin{frame}{Real-world example: Kazakhstan government certificate}
	\begin{columns}[c]
			...
			\begin{itemize}[<+->]
				...
				\item \textbf{July 2019}: Government mandates certificate installation
				      \begin{itemize}
					      ...
					      \item Initial targets: Google, Facebook, X.
				      \end{itemize}
		...
		\begin{column}{0.2\textwidth}
			\imagewithcaption{tls-borat.jpg}{}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Compromised Server}
	\begin{itemize}
		...
		\item \textbf{Attack capabilities}:
		      ...
		      \end{itemize}
		\item \textbf{Silver lining}: Forward secrecy still protects \textbf{past} sessions!
		      \begin{itemize}
			      \item Ephemeral keys already deleted from memory
			      \item Attacker cannot decrypt previously recorded traffic
		      \end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Protection strategies}:
		      \begin{itemize}
			      ...
			      \item Don't install malicious browser plugins
...
\begin{frame}{Logjam: Precomputation attacks on 1024-bit groups}
	\bigimagewithcaption{logjam-1.png}{}
\end{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Attack example}:
		      \begin{itemize}
			      ...
			      \item Attacker substitutes IKE handshake with the same hash
	...
	\begin{itemize}
		...
		\item \textbf{\fstar key features}:
		      \begin{itemize}
			      ...
			      \item Compiles to efficient C code via the KreMLin compiler.
		      \end{itemize}
		...
		\item \textbf{Real-world impact}:
		      \begin{itemize}
			      \item \haclstar integrated into Firefox, Python, and the Linux kernel.
...
\begin{frame}{Formal verification benefits for TLS 1.3}
	\begin{itemize}
		...
		\item \textbf{Industry adoption}:
		      \begin{itemize}
			      \item \haclstar used in Firefox and the Linux kernel.
