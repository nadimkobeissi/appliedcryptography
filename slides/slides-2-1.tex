	...
	\begin{itemize}[<+->]
		...
		\item A comprehensive survey of \textbf{TLS 1.2 attacks}:
		      \begin{itemize}
			      ...
			      \item SMACK (State Machine AttaCKs)
...
\begin{frame}{TLS attacks timeline (to 2015)}
	\bigimagewithcaption[0.72\textwidth]{tls-attacks-timeline-2015.png}{TLS attacks up until 2015.}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item Instant messaging (Discord, Slack)
				\item Video conferencing (Zoom, Teams)
				\item Online banking
				\item Social media (Facebook, X)
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}[<+->]
				...
				\item Observes and records traffic
			...
			\begin{itemize}[<+->]
				...
				\item Examples:
				      \begin{itemize}
					      ...
					      \item Malicious Wi-Fi hotspots
					      \item Compromised routers
				      \end{itemize}
			\end{itemize}
		\end{column}
	\end{columns}
	\begin{alertblock}{Why this distinction drives TLS design}
		Encryption alone stops a passive attacker. Defeating an \textbf{active} attacker is what forces TLS to authenticate the parties, not just encrypt.
	\end{alertblock}
\end{frame}

\begin{frame}{Key security goal: defeating man-in-the-middle attacks}
	\begin{itemize}[<+->]
		\item \textbf{MITM attack scenario} (unauthenticated key exchange, from Topic 1.7):
		      \begin{enumerate}
			      \item Attacker runs a \textbf{separate} key exchange with each side.
			      \item Decrypts, reads, re-encrypts with the other key.
			      \item Both sides believe they share a key with each other.
		      \end{enumerate}
...
\begin{frame}{Four requirements for wide TLS adoption}
	\begin{itemize}[<+->]
		...
		\item \textbf{Versatility}
		      \begin{itemize}
			      \item Not bound to specific applications: like TCP, TLS doesn't care what runs on top of it.
		      \end{itemize}
	\end{itemize}
	\pause
	\begin{center}
		Keep these four in mind: nearly every attack we'll see is the price of some legacy feature that served one of them.
	\end{center}
\end{frame}

\begin{frame}{Notice how we're mentioning design requirements now?}
	\bigimagewithcaption[0.82\textwidth]{fischer.png}{Fischer et al., USENIX Security 2024.}
\end{frame}

\begin{frame}{Notice how we're mentioning design requirements now?}
	\bigimagewithcaption[0.82\textwidth]{fischer-sectioned.png}{Fischer et al., USENIX Security 2024.}
\end{frame}
...
\begin{frame}{Fantastic resource on understanding TLS}{And secure channel protocol design in general}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{tls-illustrated}{}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Four components of a cipher suite}:
		      \begin{enumerate}
			      ...
			      \item \textbf{Hash function}: Used with HMAC to derive keys, and as the record MAC in CBC suites (SHA-256, SHA-384)
		      \end{enumerate}
		\item \textbf{Example}: \texttt{TLS\_ECDHE\_RSA\_WITH\_AES\_128\_GCM\_SHA256}
		      \begin{itemize}
			      ...
			      \item \texttt{AES\_128\_GCM}: AES-128 in GCM mode (AEAD: encryption + integrity in one)
			      \item \texttt{SHA256}: SHA-256 for the key-derivation function
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{The TLS handshake: basic flow}
	\begin{columns}[c]
		...
		\begin{column}{0.4\textwidth}
			\imagewithcaption{tls-handshake.png}{TLS handshake overview.}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{ClientHello: ``I want to establish a TLS connection''}
	\begin{itemize}[<+->]
		...
		\item \textbf{Format requirement}: Must follow the exact byte format from the TLS 1.3 spec.
		      \begin{itemize}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Configuration decisions}:
		      \begin{itemize}
			      ...
			      \item 32-byte random value
		      \end{itemize}
		\item \textbf{Authentication materials} (separate handshake messages, encrypted):
		      \begin{itemize}
			      \item Certificate (contains server's public key)
			      \item Signature over the handshake transcript so far
			      \item MAC (Finished) over everything exchanged
		      \end{itemize}
		\item \textbf{Crypto magic}: MAC uses a key derived from the DH shared secret!
	\end{itemize}
\end{frame}
...
\begin{frame}{Real-world example: visiting \texttt{aub.edu.lb}}
	...
	\begin{itemize}
		...
		\item Must be signed by a trusted certificate authority
...
\begin{frame}{Critical step: certificate validation}
	\begin{itemize}[<+->]
		\item \textbf{The crux of TLS security}: Server authenticates itself to the client
...
\begin{frame}{Let's Encrypt: revolution in TLS certificates (2015)}
	\begin{itemize}[<+->]
		...
		\item \textbf{ACME protocol} (Automated Certificate Management Environment):
		      \begin{itemize}
			      \item Prove control of a domain via HTTP or DNS challenges, programmatically.
			      \item Certificate issued in seconds, not days; renewed automatically.
		      \end{itemize}
		\item \textbf{Impact on the web}:
		      \begin{itemize}
			      \item HTTPS adoption jumped from 40\% to 90\%+ of web traffic.
			      \item Eliminated the cost barrier for small websites.
		      \end{itemize}
...
\begin{frame}{Let's Encrypt: active certificates}
	\bigimagewithcaption[0.80\textwidth]{letsencrypt-certs.png}{Source: Let's Encrypt}
\end{frame}

\begin{frame}{Let's Encrypt: certificates issued}
	\bigimagewithcaption[0.78\textwidth]{letsencrypt-issuance.png}{Source: Let's Encrypt}
\end{frame}

\begin{frame}{Percentage of HTTPS traffic in Firefox}
	\bigimagewithcaption[0.76\textwidth]{letsencrypt-https.png}{Source: Let's Encrypt}
\end{frame}
...
\begin{frame}{Certificate chains in practice}
	\begin{itemize}[<+->]
		...
		\item \textbf{Verification process}:
		      \begin{enumerate}
			      \item Check Google Trust Services' signature on Certificate 0.
...
\begin{frame}{TLS: the handshake protocol}
	\bigimagewithcaption[0.56\textwidth]{tls-13-handshake}{TLS 1.3 handshake protocol. Source: Serious Cryptography}
\end{frame}
...
\begin{frame}{Security guarantees after successful TLS handshake}
	\begin{itemize}
		...
		\item \textbf{What an eavesdropper CANNOT do}:
		      ...
		      \end{itemize}
		\item \textbf{Authentication guarantee}: If messages are tampered with, the receiving party will detect it!
...
\begin{frame}{Forward secrecy: protecting past communications}
	\begin{itemize}[<+->]
		...
		\item \textbf{Critical for}: Journalists, activists, whistleblowers, and ordinary citizens.
	\end{itemize}
\end{frame}
...
\begin{frame}{Real-world example: Kazakhstan government certificate}
	\begin{columns}[c]
		\begin{column}{0.8\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{2015}: Kazakhstan creates a ``national security certificate''
				      \begin{itemize}
					      \item A root certificate to intercept HTTPS, installed manually on users' devices.
				      \end{itemize}
				\item \textbf{July 2019}: government mandates installation
				      \begin{itemize}
					      \item ISPs push the ``Qaznet Trust Certificate''; first targets: Google, Facebook, X.
				      \end{itemize}
				\item \textbf{August 2019}: browser vendors fight back
				      \begin{itemize}
					      \item Firefox, Chrome, and Safari all block the certificate outright.
				      \end{itemize}
				\item \textbf{December 2020}: government tries again, browsers block again.
			\end{itemize}
		\end{column}
		\begin{column}{0.2\textwidth}
			\imagewithcaption[0.85\textwidth]{tls-borat.jpg}{}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Key advantages}:
				      \begin{itemize}
					      \item 10x+ cheaper ($\approx$\$10k/year vs. traditional logs), and simpler, so more reliable.
				      \end{itemize}
				\item \textbf{Sunlight}:
				      \begin{itemize}
				      ...
				      \end{itemize}
				\item \textbf{Status}: as of 2025, Chrome and Safari accept Static CT API logs.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{Discussion: is the CA model the best we can do?}
	\begin{enumerate}
		\item \textbf{You trust every CA, not one.}
		      \begin{itemize}
			      \item Any of the $\sim$150 CAs in your browser can issue a certificate for \emph{any} domain. CT makes misissuance visible \emph{after the fact}. Is ``detect, don't prevent'' good enough?
		      \end{itemize}
		\item \textbf{Are browser vendors neutral referees?}
		      \begin{itemize}
			      \item A handful of companies (Google, Mozilla, Apple, Microsoft) decide which CAs live or die. That stopped DigiNotar and Kazakhstan. What happens if a government leans on a vendor directly?
		      \end{itemize}
		\item \textbf{Could we decentralize trust?}
		      \begin{itemize}
			      \item DANE (pin the certificate in DNSSEC), a PGP-style web of trust, or blockchain names (Namecoin, ENS). Each trades the CA's single point of failure for new problems: deployment, key loss, governance. Which would you actually ship?
		      \end{itemize}
	\end{enumerate}
\end{frame}
...
\begin{frame}{Compromised server}
	\begin{itemize}[<+->]
		\item \textbf{Worst case}: the server is the TLS termination point, so an attacker who controls it gets:
		      \begin{itemize}
			      \item Session keys, and all data before encryption / after decryption.
			      \item The server's long-term private key $\rightarrow$ impersonate it anywhere.
		      \end{itemize}
		\item \textbf{Silver lining}: forward secrecy still protects \textbf{past} recorded sessions --- those ephemeral keys are already gone.
		\item \textbf{Partial mitigation}: keep the private key off the server in a \textbf{Hardware Security Module (HSM)}, so even a full compromise may not leak it.
		\item \textbf{Reality check}: the more common breaches (SQL injection, XSS) ride \emph{over} a perfectly good TLS connection --- TLS was never the thing protecting you there.
		\item \textbf{Bottom line}: TLS won't save you if the server itself is owned.
	\end{itemize}
\end{frame}

\begin{frame}{Compromised client}
	\begin{itemize}[<+->]
		\item \textbf{Scenario}: the browser or its machine is compromised (malware, a malicious extension).
		\item \textbf{What the attacker gets}: session keys and every plaintext the client sees --- it decrypts \emph{before} the attacker even needs to.
		\item \textbf{The TLS-specific move}: add a \textbf{rogue CA} to the client's trust store.
		      \begin{itemize}
			      \item The client then silently accepts the attacker's certificates.
			      \item Seamless man-in-the-middle for \emph{every} site (this is exactly what the Kazakhstan certificate asked users to do by hand).
		      \end{itemize}
		\item \textbf{Scope}: compromised CA or server affects \textbf{all} clients; a compromised client affects \textbf{only that one}.
		\item \textbf{Defenses}: keep the OS/browser patched, watch the trust store, and (for developers) certificate pinning.
	\end{itemize}
\end{frame}

\begin{frame}{Three ways the TLS trust model can break}
	\begin{center}
...
\begin{frame}{Lucky Thirteen (2013)}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{Target}: TLS's AES-CBC (Cipher Block Chaining) mode with HMAC
				...
				\item \textbf{Attack mechanism}:
				      ...
				      \end{enumerate}
				\item \textbf{Why ``Lucky Thirteen''?} The TLS MAC computation includes a 13-byte header
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{itemize}
		\item \textbf{CBC padding in TLS}:
		      \begin{itemize}
			      ...
			      \item Example: \texttt{...data|05|05|05|05|05|05} (6 padding bytes, length = 5)
		      \end{itemize}
		\item \textbf{The timing leak}:
		      \begin{itemize}
			      \item \textbf{Valid padding}: MAC checked on message after padding removal
			      \item \textbf{Invalid padding}: TLS spec says to check MAC as if the padding had zero length
			      \item Different amounts of data processed create a timing difference
		      \end{itemize}
...
\begin{frame}{Lucky Thirteen: real-world impact}
	\begin{itemize}
		...
		\item \textbf{Practical exploitation}:
		      \begin{itemize}
			      \item Requires a man-in-the-middle position
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{The setup}: browsers still support SSL 3.0 ``for compatibility'', and its padding validation is weaker than TLS's.
				\item \textbf{Attack flow}:
				      \begin{enumerate}
					      \item Client tries TLS 1.2; attacker blocks/corrupts the handshake.
					      \item Client ``gracefully'' falls back to SSL 3.0.
					      \item Attacker exploits the SSL 3.0 padding oracle.
				      \end{enumerate}
...
\begin{frame}{POODLE: the padding oracle vulnerability}
	\begin{itemize}
		...
		\item \textbf{Attack technique}:
		      \begin{itemize}
			      \item Replace the last ciphertext block with a target block.
			      \item Valid padding $\rightarrow$ server accepts; invalid $\rightarrow$ error. That is the oracle.
			      \item Guess plaintext bytes one at a time.
		      \end{itemize}
		\item \textbf{Efficiency}: extract one byte per 256 requests on average.
...
\begin{frame}{Triple Handshakes: the technical attack}
	\begin{itemize}
		...
		\item \textbf{``Triple handshake'' attack}:
		      \begin{enumerate}
			      \item Handshake 1: Client \leftrightarrow\ Attacker (using evil certificate)
			      \item Handshake 2: Attacker \leftrightarrow\ Server (using good certificate)
			      \item Handshake 3: Client \leftrightarrow\ Server (resumed session, confused identity)
		      \end{enumerate}
		\item \textbf{Result}: Client sends sensitive data to the attacker
		\item \textbf{Renegotiation attacks}: Similar issues with TLS renegotiation
		\item \textbf{Cookie cutter}: Attacker truncates TLS records to strip security attributes from cookies
	\end{itemize}
\end{frame}
...
\begin{frame}{Heartbleed (2014): the bug that broke the internet}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{Not a protocol flaw}: implementation bug in OpenSSL (CVE-2014-0160)\footnote{\url{https://appliedcryptography.page/paper/\#matter-heartbleed}}
	...
	\begin{itemize}
		\item \textbf{Server's private keys}:
		      \begin{itemize}
			      ...
			      \item Active session keys leaked (forward secrecy protects past sessions)
		      \end{itemize}
...
\begin{frame}{Heartbleed: global impact and response}
	\begin{itemize}
		...
		\item \textbf{Long-term consequences}:
		      ...
		      \end{itemize}
		\item \textbf{Branding success}: First security vulnerability with its own logo and website
	\end{itemize}
\end{frame}
...
\begin{frame}{SMACK: how state machine attacks work}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{tls-smack.png}{TLS state machine}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{SMACK: a concrete two-library example}
	\begin{itemize}[<+->]
		...
		\item \textbf{Attack technique}:
		      \begin{enumerate}
			      ...
			      \item Library A thinks the handshake is complete
			      \item Library B thinks the handshake is still in progress
		      \end{enumerate}
		\item \textbf{Result}:
		      \begin{itemize}
...
\begin{frame}{``The Crypto Wars''}
	\begin{columns}[c]
		\begin{column}{0.33\textwidth}
			\imagewithcaption{export-1.jpg}{RSA tattoo}
		\end{column}
		\begin{column}{0.33\textwidth}
			\imagewithcaption{export-2.jpg}{RSA t-shirt}
		\end{column}
		\begin{column}{0.33\textwidth}
			\imagewithcaption{export-3.jpg}{Netscape ``not for export'' floppy}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{A Cypherpunk's Manifesto (1993)}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}[<+->]
...
\begin{frame}{Logjam: the downgrade attack}
	\begin{itemize}
		...
		\item \textbf{Breaking 512-bit DH}:
		      \begin{itemize}
			      \item One week of precomputation per 512-bit prime (about 10 core-years), done once
			      \item Then each individual log takes about 70 seconds (median) on one 36-core machine
			      \item Two primes account for 92\% of vulnerable servers, so two weeks of work broke almost all of them
		      \end{itemize}
		\item \textbf{Vulnerable servers}: 8.4\% of top 1 million HTTPS sites
		\item \textbf{The irony}: Export restrictions from the 1990s still creating vulnerabilities in 2015
	\end{itemize}
\end{frame}

\begin{frame}{Logjam: the downgrade attack}
	\bigimagewithcaption{logjam-2.png}{}
\end{frame}
...
\begin{frame}{Logjam: precomputation attacks on 1024-bit groups}
	\begin{itemize}
		...
		\item \textbf{Attack economics}:
		      \begin{itemize}
			      \item Precomputation for one 1024-bit group: about 45 million core-years; roughly a year with a few hundred million dollars of custom hardware
			      \item Individual discrete logs afterwards: about 30 core-days each
...
\begin{frame}{Logjam: precomputation attacks on 1024-bit groups}
	\bigimagewithcaption[0.95\textwidth]{logjam-1.png}{}
\end{frame}
...
\begin{frame}{Logjam: intelligence services exploit these issues!}
	\bigimagewithcaption{logjam-3.png}{}
\end{frame}
...
\begin{frame}{SWEET32: birthday attacks on 64-bit block ciphers (2016)}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{Researchers}: Karthikeyan Bhargavan and Ga\"etan Leurent (Inria)\footnote{\url{https://appliedcryptography.page/paper/\#inria-sweet32}}
	...
	\begin{itemize}
		\item \textbf{Birthday paradox}:
		      \begin{itemize}
			      ...
			      \item At 1 MB/s: $\approx$9.5 hours, at 10 MB/s: $\approx$1 hour
		      \end{itemize}
		\item \textbf{Collision exploitation}:
		      \begin{itemize}
			      \item When two ciphertext blocks collide \rightarrow\ reveals XOR of plaintext blocks
...
\begin{frame}{Transcript collision attacks (2016)}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{Researchers}: Karthikeyan Bhargavan and Ga\"etan Leurent (Inria)\footnote{\url{https://appliedcryptography.page/paper/\#inria-collisions}}
				...
				\item \textbf{Core idea}:
				      \begin{itemize}
					      ...
					      \item Find two different transcripts with the same hash
...
\begin{frame}{Transcript collision}
	\bigimagewithcaption{tls-collision.png}{}
\end{frame}
...
\begin{frame}{Transcript collision: attack on TLS}
	\begin{itemize}
		...
		\item \textbf{Attack scenario}:
		      \begin{enumerate}
			      \item Attacker finds two handshake transcripts with the same hash
...
\begin{frame}{Transcript collision: cross-protocol attacks}
	\begin{itemize}
		...
		\item \textbf{Attack example}:
		      \begin{itemize}
			      \item A signature the server computes over a TLS transcript is made to also validate an attacker's IKE (or SSH) transcript with the same hash.
			      \item One protocol's credential is thus replayed to authenticate in another --- breaking authentication across protocols.
		      \end{itemize}
	...
	\begin{itemize}
		\item \textbf{Authenticated Encryption} (only 3 algorithms):
		      \begin{itemize}
			      ...
			      \item ChaCha20-Poly1305 (256-bit keys, from RFC 8439)
		      \end{itemize}
		...
		\item \textbf{Diffie-Hellman Key Exchange}:
		      ...
		      \end{itemize}
		\item \textbf{Security note}: 2,048-bit DH provides $\approx$112-bit security
		      \begin{itemize}
			      \item Falls short of 128-bit security target
...
\begin{frame}{Project Everest: verified cryptographic implementations}
	\begin{itemize}
		...
		\item \textbf{\haclstar cryptographic library}:
		      ...
		      \end{itemize}
		\item \textbf{miTLS}: verified reference implementation of TLS
		      \begin{itemize}
			      \item TLS 1.2, plus the TLS 1.3 record layer, with security proofs
...
\begin{frame}{ProVerif: automated protocol verification}
	\begin{itemize}
		...
		\item \textbf{How ProVerif works}:
		      \begin{itemize}
			      \item Protocol modeled in the applied pi-calculus.
	...
	\begin{itemize}
		...
		\item \textbf{\fstar key features}:
		      \begin{itemize}
			      ...
			      \item Compiles to efficient C via the KaRaMeL compiler (formerly KreMLin).
		      \end{itemize}
		\item \textbf{TLS 1.3 in \fstar}:
		      \begin{itemize}
			      \item \textbf{miTLS}: verified reference implementation (TLS 1.2 + the 1.3 record layer).
		      ...
		      \end{itemize}
		\item \textbf{Real-world impact}: \haclstar ships in Firefox, Python, and the Linux kernel --- verified code can be practical and fast.
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		\item \textbf{TLS 1.2 feature}: Optional data compression
		      \begin{itemize}
			      ...
			      \item Seemed like a good idea\ldots
		      \end{itemize}
	...
	\begin{itemize}
		\item \textbf{Traffic analysis attack}:
		      \begin{itemize}
			      ...
			      \item Ciphertext size $\approx$ plaintext size (reveals message length).
		      \end{itemize}
...
\begin{frame}{Downgrade protection: preventing version rollback}
	\begin{itemize}
		...
		\item \textbf{TLS 1.3 defense}: Magic values in server random
		      \begin{itemize}
			      \item Server encodes connection type in last 8 bytes of random value
			      \item TLS 1.2: \texttt{44 4F 57 4E 47 52 44 01}
			      \item TLS 1.1 or below: \texttt{44 4F 57 4E 47 52 44 00}
...
\begin{frame}{The magic downgrade detection values}
	...
	\begin{itemize}
		...
		\item \textbf{Easy debugging}: Hex dumps show the ``DOWNGRD'' string
...
\begin{frame}{Session resumption: even faster connections}
	\begin{itemize}
		\item \textbf{The idea}: Reuse keys from previous connections
		      \begin{itemize}
			      \item Client and server remember a \textbf{preshared key (PSK)}
	...
	\begin{itemize}
		...
		\item \textbf{Process}:
		      \begin{enumerate}
			      ...
			      \item Client: Verifies MAC, knows it's talking to the right server
		      \end{enumerate}
	...
	\begin{itemize}
		\item \textbf{Replay attack vulnerability}:
		      \begin{itemize}
			      ...
			      \item Server can't distinguish a replay from a legitimate connection
			            \begin{itemize}
...
\begin{frame}{Takeaways: what TLS teaches us}
	\begin{itemize}[<+->]
		\item \textbf{Every attack in this lecture was the price of a legacy feature} kept for compatibility: export ciphers (FREAK, Logjam), CBC padding (Lucky 13, POODLE), 64-bit blocks (SWEET32), weak hashes (transcript collisions), compression (CRIME). TLS 1.3's answer was to \textbf{delete the option}.
		\item \textbf{Authentication, not just encryption}, is what defeats an active attacker --- and the trust it rests on (CAs, the client, the server) is only as strong as its weakest party.
		\item \textbf{TLS 1.3 was designed alongside its proofs}, not patched afterward. That playbook now shapes Signal, WireGuard, and post-quantum protocols.
		\item \textbf{Where this goes next in the course}: RC4's long death (2.2), the Signal protocol (2.3), and the verification tools behind TLS 1.3 in depth (2.5).
	\end{itemize}
\end{frame}
