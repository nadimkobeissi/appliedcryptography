...
\begin{frame}{Elliptic curves over finite fields}
	\begin{itemize}
		\item \textbf{Practical implementation:} Most elliptic curve cryptosystems work with coordinates modulo a prime $p$.
		      ...
		      \end{itemize}
		\item \textbf{Why finite fields?} Infinitely precise real numbers are impractical for computers: finite-field arithmetic is exact, efficient, and discrete.
		\item \textbf{Security foundation:} Security depends on the curve's \emph{cardinality} (its number of points), which is determined by $a$, $b$, and $p$.
		      \begin{itemize}
			      \item More points $\rightarrow$ harder discrete logarithm problem (like larger moduli in RSA).
		      \end{itemize}
		\item \textbf{Order of a point:} The smallest $n > 0$ with $n \cdot P = \mathcal{O}$: keep adding $P$ to itself, and after $n$ steps you cycle back to where you started.
		      \begin{itemize}
			      \item The base point $G$ generates a cyclic subgroup $\{G, 2G, \ldots, n \cdot G = \mathcal{O}\}$—the ``playground'' where our cryptography happens.
		      \end{itemize}
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		\item \textbf{Ed25519 = EdDSA + specific parameters:}
		      \begin{itemize}
			      ...
			      \item A fixed, transparently chosen base point
		      \end{itemize}
		\item \textbf{Performance characteristics:}
		      \begin{itemize}
			      \item Signing: $\approx$40--90 microseconds (modern CPUs)
			      \item Verification: $\approx$100--200 microseconds
			      \item 64-byte signatures (512 bits)
		      \end{itemize}
		\item \textbf{Security level:} $\approx$128 bits (equivalent to 3,072-bit RSA)
		\item \textbf{Adoption milestones:}
		      \begin{itemize}
			      \item 2011: Initial specification
			      \item 2014: OpenSSH support
