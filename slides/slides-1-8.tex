...
\begin{frame}{Elliptic-curve cryptography}
	\begin{itemize}
		...
		\item \textbf{Superior efficiency:} More powerful than RSA and classical Diffie-Hellman.
		      \begin{itemize}
			      \item ECC with 256-bit key $\approx$ RSA with 3,072-bit key (security).
...
\begin{frame}{Elliptic curves over real numbers vs. finite fields}
	\begin{columns}
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{elliptic-curve-integers.png}{Same elliptic curve over a finite field (only whole positive numbers)}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Group structure of elliptic curves}
	We need to define all these operations so that our elliptic curves have a group structure, allowing us to then use them as a new basis for Diffie-Hellman, and then do DH using point addition instead of modular multiplication.
	\begin{itemize}
...
\begin{frame}{ECDSA: The established standard}
	\begin{itemize}
		\item \textbf{Elliptic Curve Digital Signature Algorithm (ECDSA):}
		      \begin{itemize}
			      \item Proposed in 1992, ANSI standardized in 1999, NIST in 2000.
...
\begin{frame}{Ed25519: The practical implementation}
	\begin{itemize}
		...
		\item \textbf{Performance characteristics:}
		      ...
		      \end{itemize}
		\item \textbf{Security level:} $\approx$128 bits (equivalent to 3072-bit RSA)
...
\begin{frame}{Real-world case study: TLS 1.3 performance}
	\begin{itemize}
		...
		\item \textbf{Implementation impact:}
		      \begin{itemize}
			      \item X25519 ECDH: significantly faster than 2048-bit RSA key exchange.
