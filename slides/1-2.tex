...
\begin{document}
	...
	\begin{frame}{Key derivation}{Uniform distribution}
		\begin{itemize}
			...
			\item Sampling a $K$ from a pseudo-random uniform distribution is written as $K \twoheadleftarrow \{\nliteral{0}, \nliteral{1}\}^n$
		\end{itemize}
	\end{frame}
		...
		\begin{center}
			\begin{tikzpicture}[
				...
				\node[box, right=2cm of adversary](attack){
					...
					\quad $K \twoheadleftarrow \{\nliteral{0}, \nliteral{1}\}^{n}$ \hfill \quad \quad \textcolor{gray}{\scriptsize \textit{// victim samples $K$}} \\
					\quad $C \coloneq Enc(K, M)$ \hfill \quad \quad \textcolor{gray}{\scriptsize \textit{// victim encrypts}} \\
	...
	\begin{frame}{One-time pad}{Correctness proof}
		\begin{itemize}
			\item $\forall(n > 0,\; K \in \{\nliteral{0}, \nliteral{1}\}^{n},\; M\in\{\nliteral{0}, \nliteral{1}\}^{n}),\; Dec(K, Enc(K, M)) = M$
		...
		\begin{columns}[c]
			\begin{column}{0.7\textwidth}
				\begin{itemize}
					...
					\item Suppose $M = \nliteral{01}$:
					\begin{itemize}
						\item $K = \nliteral{00}$ is chosen with probability $1/4$:\\$C = K \oplus M = \nliteral{00} \oplus \nliteral{01} = \nliteral{01}$.
						\item $K = \nliteral{01}$ is chosen with probability $1/4$:\\$C = K \oplus M = \nliteral{01} \oplus \nliteral{01} = \nliteral{00}$.
						\item $K = \nliteral{10}$ is chosen with probability $1/4$:\\$C = K \oplus M = \nliteral{10} \oplus \nliteral{01} = \nliteral{11}$.
						\item $K = \nliteral{11}$ is chosen with probability $1/4$:\\$C = K \oplus M = \nliteral{11} \oplus \nliteral{01} = \nliteral{10}$.
					\end{itemize}
				\end{itemize}
			\end{column}
	...
	\begin{frame}{One-time pad}{How do we prove security?}
		\begin{columns}[c]
			\begin{column}{0.7\textwidth}
				\begin{itemize}
					\item What if this is true only for $M = \nliteral{01}$?
					\pause
					\item Fine, let's pick any $M, C \in \{\nliteral{0, 1}\}^n$.
					\pause
					\item What is \prob{$ATTACK(M) = C$}?
					\pause
					\item Answer: Exactly when $C = Enc(K, M) = K \oplus M$.
					\pause
					\item ...which occurs for exactly one $K$.
					\pause
					\item Since $K$ is chosen uniformly from $\{\nliteral{0}, \nliteral{1}\}^n$, the probability of choosing that $K$ is $\frac{1}{2^n}. \quad \qed$
				\end{itemize}
			\end{column}
			\begin{column}{0.3\textwidth}
				\sssubroutine{$ATTACK(M)$}{
	...
	\begin{frame}{One-time pad}{What's so special about XOR?}
		\begin{columns}[c]
			\begin{column}{0.7\textwidth}
				\begin{itemize}
					\item<1-> Let's replace $\oplus$ with $\land$. What would happen?
					\item<2-> Output no longer uniform!
				\end{itemize}
				\begin{table}
					\centering
					\begin{tabular}{|c|c|c|}
