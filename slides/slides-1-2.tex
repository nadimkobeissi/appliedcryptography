...
\begin{frame}{Key derivation}{Uniform distribution}
	\begin{itemize}[<+->]
		...
		\item True randomness is hard to obtain, so in practice $K$ is pseudo-random.
		...
		\item In practice, this just means we need the bits to be random, unpredictable, and uniformly distributed
...
\begin{frame}{One-time pad}{Correctness proof}
	\begin{itemize}[<+->]
		...
		\item For all positive $n$: encrypting any $n$-bit message under any $n$-bit key, then decrypting under the same key, returns the original message.
		\item \textbf{Proof}:
		      \begin{align*}
...
\begin{frame}{One-time pad}{How do we prove security?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item When we prove security, we prove what is or isn't possible for an attacker who calls \textsc{Attack}$(M)$.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}[<+->]
				\item ``Victim'' chooses their key.
				      \begin{itemize}
					      ...
					      \item This means the output will differ even if the adversary inputs the same plaintext twice
				      \end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item Two queries with the same $M$ (almost always) return different $(C, C')$, since the victim samples a fresh $K$ each time.
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item \textbf{Formally}: For all positive integers $n$ and all choices of plaintext $M \in \bits^n$, the output of the following subroutine is uniformly distributed:
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
...
\begin{frame}{One-time pad}{What about addition modulo $n$?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item Let's replace $\oplus$ with addition modulo $n$. What would happen?
				\item Still good!
	...
	\begin{itemize}[<+->]
		\item Despite limitations, security proofs provide important benefits:
		      \begin{itemize}[<+->]
			      ...
			      \item \textbf{Confidence}: A valid proof rules out all attacks within the model.
...
\begin{frame}{Key distribution problem}{The elephant in the room}
	\begin{columns}[c]
		\column{0.55\textwidth}{
			\begin{itemize}[<+->]
				\item OTP requires a key as long as the message
				...
				\item \textbf{Historical solutions}:
				      ...
				      \end{itemize}
				\item \textbf{Modern issue}: Impractical at internet scale
			\end{itemize}
		}
