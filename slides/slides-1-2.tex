		...
		\column{0.5\textwidth}{
			\begin{center}
				\begin{tikzpicture}[scale=1.2]
					...
					\fill (0,0) circle (2pt);
					\node[below] at (0,-2.5cm) {\small Example: 16:00 = 4:00pm};
				\end{tikzpicture}
			\end{center}
		}
	\end{columns}
...
\begin{frame}{Interactive demo}{One-time pad attack}
	\begin{columns}[c]
		\column{1\textwidth}{
			\begin{itemize}[<+->]
				\item \textbf{Given}: $C_1$, $C_2$ (same key)
				\item \textbf{Step 1}: Compute $C_1 \oplus C_2 = M_1 \oplus M_2$
				\item \textbf{Step 2}: Guess common word in $M_1$
				\item \textbf{Step 3}: XOR with $M_1 \oplus M_2$ to get potential $M_2$ fragment
				\item \textbf{Step 4}: If sensible, expand the guess
				\item \textbf{Step 5}: Iterate until messages recovered
			\end{itemize}
		}
	\end{columns}
\end{frame}

\begin{frame}{Historical example}{Project Venona}
	\begin{columns}[c]
		\column{1\textwidth}{
			\begin{itemize}[<+->]
				\item \textbf{1940s-1950s}: Soviet intelligence used one-time pads
				\item \textbf{The mistake}: Key pages were accidentally duplicated
				\item \textbf{Result}: Some messages encrypted with same key
				\item \textbf{NSA/GCHQ response}: Decades-long decryption effort
				\item \textbf{Impact}: Exposed Soviet spy networks, including:
				      \begin{itemize}[<+->]
					      \item Klaus Fuchs (atomic secrets)
					      \item Julius and Ethel Rosenberg
					      \item Cambridge Five spy ring
				      \end{itemize}
				\item \textbf{Lesson}: Even ``perfect'' cryptography fails with improper use
			\end{itemize}
		}
	\end{columns}
\end{frame}
