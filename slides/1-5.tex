...
\coversessionname{1.5: Chosen-Plaintext \& Chosen-Ciphertext Attacks}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\sslinked{
				\ssprogram{}{
					$M \twoheadleftarrow \mathcal{M}$\\
					$R_1\|S_1 \coloneq \sssubroutinename{cpa.enc}{M}$\\
					$R_2\|S_2 \coloneq \sssubroutinename{cpa.enc}{M}$\\
...
\begin{frame}{CBC mode: a closer look}
	\vspace{-0.3cm}
	\bigimagewithcaption{cbc_mode.png}{Source: The Joy of Cryptography}
\end{frame}
...
\begin{frame}{CTR mode: a closer look}
	\bigimagewithcaption{ctr_mode.png}{Source: The Joy of Cryptography}
\end{frame}
...
\begin{frame}{Why is it called \sssubroutinename{nulloracle}{c}?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
...
\begin{frame}{Why is it called \sssubroutinename{nulloracle}{c}?}
	\begin{itemize}[<+->]
		\item In cryptography and computer science:
		      ...
		      \end{itemize}
		\item Our \sssubroutinename{nulloracle}{c} reveals only:
		      \begin{itemize}
...
\begin{frame}{Null-oracle attack}
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\begin{itemize}[<+->]
				\item An adversary who has access to \sssubroutinename{nulloracle}{C} can efficiently compute \texttt{Dec}$(K, C)$ for any $C$!
...
\begin{frame}{CTR mode is malleable}{Defining malleability}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{ctr_dec.png}{CTR mode decryption.\\Source: The Joy of Cryptography}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{CTR mode is malleable}{Defining malleability}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{ctr_dec_delta.png}{CTR mode decryption.\\Source: The Joy of Cryptography}
		\end{column}
	\end{columns}
\end{frame}
			...
			\begin{itemize}[<+->]
				...
				\item For each guess $g$, we:
				      \begin{itemize}
					      ...
					      \item Send $C'$ to the \sssubroutinename{nulloracle}{}
...
\begin{frame}{CTR mode is malleable}{Finding all bytes}
	\begin{columns}[c]
		...
		\begin{column}{0.4\textwidth}
			...
			$\quad\quad$ $\Delta[i] := b$\\
			$\quad\quad$ if $\sssubroutinename{nulloracle}{C \oplus (0^\lambda \| \Delta)}$:\\
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}[<+->]
				...
				\item Example: POODLE attack against SSL 3.0 affected millions of websites.\footnote{Check out the full paper, available in the \textit{Online Readings} section of the course website: \url{https://appliedcryptography.page}}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}[<+->]
				...
				\item Example: Lucky Thirteen attack against TLS revealed message contents through timing differences.\footnote{Check out the full paper, available in the \textit{Online Readings} section of the course website: \url{https://appliedcryptography.page}}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}[<+->]
				...
				\item System responded differently when a ciphertext decrypted to:
				      ...
				      \end{itemize}
				\item This created an oracle revealing: ``Is $\texttt{Dec}(K, C)$ a valid gzip file?''\footnote{Check out the full paper, available in the \textit{Online Readings} section of the course website: \url{https://appliedcryptography.page}}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\ssprogram{}{
				...
				$R\|S \coloneq \sssubroutinename{cca.enc}{M}$\\
				$M' \coloneq \sssubroutinename{cca.dec}{R\|(S \oplus \Delta)}$\\
...
\begin{frame}{Another example}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\ssprogram{}{
				$M \twoheadleftarrow \bits^\lambda$\\
				$R_1\|S_1 \coloneq \sssubroutinename{cca.enc}{M}$\\
				$R_2\|S_2 \coloneq \sssubroutinename{cca.enc}{M}$\\
				$M_1^* \coloneq \sssubroutinename{cca.dec}{R_1\|S_2}$\\
				$M_2^* \coloneq \sssubroutinename{cca.dec}{R_2\|S_1}$\\
				return $M_1^* == M_2^*$
			}{1}
		\end{column}
	\end{columns}
...
\begin{frame}{Replay attacks}{Step 1: Alice sends original message}
	\bigimagewithcaption{aead_scenario_1.png}{Source: The Joy of Cryptography}
\end{frame}

\begin{frame}{Replay attacks}{Step 2: Attacker replays message in a different context}
	\bigimagewithcaption{aead_scenario_2.png}{Source: The Joy of Cryptography}
\end{frame}
...
\begin{frame}{Solution: use associated data to provide context}
	\bigimagewithcaption{aead_scenario_4.png}{Source: The Joy of Cryptography}
\end{frame}
