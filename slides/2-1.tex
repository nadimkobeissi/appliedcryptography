			...
			\begin{itemize}[<+->]
				...
				\item \textbf{The vulnerability}: Timing differences in MAC verification\footnote{\url{https://appliedcryptography.page/papers/lucky-thirteen.pdf}}
				      \begin{itemize}
					      ...
					      \item Server takes different time to process valid vs. invalid padding
				      \end{itemize}
				\item \textbf{Attack mechanism}:
				      \begin{enumerate}
					      \item Attacker modifies ciphertext to create invalid padding
					      \item Measures server response time
					      \item Fast response = padding error, slow response = MAC error
...
\begin{frame}{Lucky Thirteen: Technical details}
	\begin{itemize}[<+->]
		...
		\item \textbf{The timing leak}:
		      \begin{itemize}
			      \item \textbf{Invalid padding}: Server immediately returns error
			      \item \textbf{Valid padding}: Server computes MAC, then returns error
			      \item MAC computation takes measurable time
		      \end{itemize}
