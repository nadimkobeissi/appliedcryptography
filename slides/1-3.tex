...
\coversessionname{1.3: Provable Security \\ \& Computational Cryptography}
...
\begin{frame}{Subroutines}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{attacker_interface.png}{Source: The Joy of Cryptography}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Programs and libraries}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\sslinked{
				\ssprogram{}{
					if \sssubroutinename{guess}{6}:\\
					\quad return true\\
					\sssubroutinename{reroll}{}\\
					return \sssubroutinename{guess}{6}
				}{0.95}
			}{\link}{
				\sslibrary{}{dice-guess}{
					$D \twoheadleftarrow \{1, 2, 3, 4, 5, 6\}$ \\[1em]
			...
			\sslinked{
				\sslibrary{}{a}{
					\sslibrarysubroutine{foo}{A, B}{
						...
						$C \coloneq \sssubroutinename{bar}{A}$\\
						return $Y$
					}{1}\\[1em]
				...
				\sslibrary{}{b}{
					\sslibrarysubroutine{foo}{A, B}{
						$Y \twoheadleftarrow \bits^n$\\
						$C \coloneq \sssubroutinename{bar}{B}$\\
						return $Y$
					}{1}\\[1em]
					\sslibrarysubroutine{bar}{M}{
						$X \twoheadleftarrow \bits^n$\\
	...
	\begin{columns}[c]
		...
		\begin{column}{0.6\textwidth}
			\begin{flushright}
				\sslinked{
					\ssprogram{}{
						$C_1 \coloneq \sssubroutinename{otp.enc}{\bit{0}^n}$\\
						$C_2 \coloneq \sssubroutinename{otp.enc}{\bit{0}^n}$\\
						return $C_1 == C_2$
					}{1}
				}{\link}{
					\sslibrary{}{a}{
			...
			\begin{flushright}
				\sslinked{
					\ssprogram{}{
						...
						$C_1 \coloneq \sssubroutinename{otp.enc}{M_1}$\\
						$C_2 \coloneq \sssubroutinename{otp.enc}{M_2}$\\
...
\begin{frame}{Proving two libraries are interchangeable}{Step 4}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item The first two lines of \sssubroutinename{sample}{M} are the same as \sssubroutinename{otp.enc}{M}, so we link it and use it instead.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{center}
				\sslinked{
					\sslibrary{}{hyb-3-4}{
						\sslibrarysubroutine{sample}{M}{
							\hl{$Y \coloneq \sssubroutinename{otp.enc}{M}$} \\
							$X' \coloneq Y \oplus M$ \\
							return $(X', Y)$
						}{1}
					}{1}
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				\sslinked{
					\sslibrary{}{hyb-3-4}{
						\sslibrarysubroutine{sample}{M}{
							$Y \coloneq \sssubroutinename{otp.enc}{M}$ \\
							$X' \coloneq Y \oplus M$ \\
							return $(X', Y)$
						}{1}
					}{1}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				...
				\item \ssprogram{}{
					      $M \coloneq \bit{0}^n$\\
					      $C \coloneq \sssubroutinename{ots.enc}{M}$\\
					      return $C == \bit{0}^n$
				      }{1}
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\begin{itemize}[<+->]
				...
				\item This probability is called $\sssubroutinename{birthday}{q,N}$
			\end{itemize}
		\end{column}
		\begin{column}{0.4\textwidth}
			\begin{align*}
				\sssubroutinename{birthday}{q,N} = 1 - \prod_{i=1}^{q-1} \left(1 - \frac{i}{N}\right)
			\end{align*}
			\begin{itemize}[<+->]
...
\begin{frame}{The ``end-of-time'' strategy for bad events}
	\begin{itemize}[<+->]
		...
		\item Advantages:
		      ...
		      \end{itemize}
		\item We'll use this in the next session!
	\end{itemize}
\end{frame}
