	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}[<+->]
				...
				\item \textbf{Example}: Factoring product of two large primes.
...
\begin{frame}{Quantum vulnerability of hard problems}
	\begin{itemize}[<+->]
		...
		\item This has motivated the search for \textbf{``post-quantum''} hard problems:
		      ...
		      \end{itemize}
		\item NIST has standardized post-quantum cryptographic algorithms to replace our vulnerable systems.
	\end{itemize}
\end{frame}
...
\begin{frame}{Funny things standardized by NIST}
	\begin{itemize}[<+->]
		...
		\item \textbf{Standard Reference Whale Blubber}: for calibrating instruments that measure environmental pollutants in marine life.
	\end{itemize}
\end{frame}
...
\begin{frame}{SPACE complexity classes}
	\begin{itemize}[<+->]
		...
		\item \textbf{SPACE}$(f(n))$ = class of problems solvable using $\mathcal{O}(f(n))$ bits of memory.
		\item Examples:
		      \begin{itemize}
...
\begin{frame}{NP: A cryptographic example}
	\textbf{Problem}: Given a hash value $H$, does there exist an input $M$ such that $\func{hash}{M} = H$?
	\begin{itemize}[<+->]
		\item \textbf{Finding the solution}: Could take exponential time (brute-force searching through possible inputs).
		\item \textbf{Verifying a candidate solution}: Given a potential input $M_0$:
		      \begin{enumerate}
			      \item Compute $\func{hash}{M_0}$
			      \item Check if $\func{hash}{M_0} = H$
			      \item Return ``yes'' if they match, ``no'' otherwise
		      \end{enumerate}
		\item This verification runs in polynomial time!
		\item Therefore, this hash preimage problem is in NP.
	\end{itemize}
\end{frame}

\begin{frame}{NP: Another cryptographic example}
...
\begin{frame}{The magic of reductions}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Mind-blowing consequence}: Solve Sudoku efficiently = solve every problem in NP efficiently!
	\end{itemize}
\end{frame}
...
\begin{frame}{What if P = NP?}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{But also}: We could solve protein folding, optimize supply chains perfectly, revolutionize climate modeling\ldots
	\end{itemize}
\end{frame}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}[<+->]
				\item Random instances are typically hard.
				      \begin{itemize}
					      \item \textbf{Example}: Finding $a$ or $b$ given $(g, g^a \bmod p, g^b \bmod p, g^{ab} \bmod p)$ (Diffie-Hellman).
					      \item \textbf{Example}: Factoring products of large primes (RSA).
				      \end{itemize}
...
\begin{frame}{What makes discrete logarithm hard?}
	\begin{itemize}[<+->]
		...
		\item \textbf{Hard direction}: Given $g$, $p$, and $g^x \bmod p$, find $x$.
		      \begin{itemize}
			      \item Example: Given $g = 2$, $p = 17$, and a result of $4$, find $x = 10$
		      \end{itemize}
...
\begin{frame}{A simple example}
	...
	\vspace{0.5cm}
	\begin{itemize}[<+->]
		\item Let $p = 17$ (a prime) and $g = 2$ (a base)
...
\begin{frame}{The computational Diffie-Hellman (CDH) problem}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Example}: Given $A = 6$ and $B = 11$ from our earlier example, can you compute $S = 7$?
		      \begin{itemize}
			      \item Without knowing $a = 5$ and $b = 7$, this becomes very difficult!
		      \end{itemize}
	...
	\begin{itemize}[<+->]
		\item \textbf{Why do we need this?} Indistinguishability!
		      \begin{itemize}
			      ...
			      \item CDH isn't completely broken, but the attacker learns something.
...
\begin{frame}{DDH vs. CDH: the hierarchy}
	\begin{itemize}[<+->]
		...
		\item \textbf{Surprising fact}: DDH is \textbf{not hard} in certain groups!
		      \begin{itemize}
			      \item In $\mathbb{Z}_p^*$, DDH can be broken using the Legendre symbol (quadratic residuosity).
...
\begin{frame}{Why DDH matters in cryptography}
	\begin{itemize}[<+->]
		...
		\item \textbf{Security proofs}: Many cryptographic protocols prove security under DDH.
		      ...
		      \end{itemize}
		\item \textbf{Real-world impact}: Even though DDH is a \textbf{stronger assumption} than CDH, it's one of the most studied and used assumptions.
		      \begin{itemize}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Problem}: $S$ is \textbf{not} a uniformly random bitstring --- it's a specific element of a specific algebraic group.
		      \begin{itemize}
			      ...
			      \item Its byte-length can vary (leading zero bytes), a classic source of real implementation bugs.
		      \end{itemize}
...
\begin{frame}{Weak groups in the wild: the Logjam findings}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{center}
				\Large\textbf{Not all Diffie-Hellman implementations are created equal}
			\end{center}
			\vspace{0.5cm}
			\begin{itemize}[<+->]
				\item The Logjam paper (2015)\footnote{\url{https://appliedcryptography.page/paper/\#imperfect-dh}} revealed widespread weaknesses in real-world DH deployments
				\item Researchers scanned millions of servers and found shocking vulnerabilities
				\item Three major categories of problems:
				      \begin{itemize}
					      \item Servers using 512-bit primes for non-export DHE.
					      \item Composite-order subgroup attacks.
					      \item Misconfigured DSA groups.
				      \end{itemize}
				\item \textbf{Observation}: Even correct algorithms fail when deployed incorrectly!
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Logjam finding: 512-bit primes still in use}
	...
	\begin{itemize}[<+->]
		\item \textbf{Why this is bad}:
		      \begin{itemize}
			      \item 512-bit discrete logarithm is solvable with modest resources
			      \item A passive eavesdropper can decrypt connections after the fact
			      \item No active attack needed—just record and decrypt later!
		      \end{itemize}
		\item \textbf{Real-world proof}: Researchers decrypted test connections to www.fbi.gov
		      \begin{itemize}
			      \item The FBI website used OpenSSL's default 512-bit group until April 2015
			      \item Researchers had precomputed the discrete log table for this group
			      \item Complete compromise of ``secure'' government communications!
		      \end{itemize}
		\item \textbf{The lesson}: Default configurations can be dangerously weak
		      \begin{itemize}
			      \item Many admins never change defaults
			      \item OpenSSL's 512-bit default was catastrophically inadequate
		      \end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}{Another Logjam finding: Composite-order subgroup attacks}
	\begin{itemize}[<+->]
		\item Not all servers use ``safe'' primes where $(p-1)/2$ is also prime
		      \begin{itemize}
			      \item 4,800 out of 70,000 primes were not safe.
			      \item If $p-1$ has small factors, the group has weak subgroups.
			      \item \textbf{Note}: Even with non-safe primes, proper generator choice can maintain security.
		      \end{itemize}
		\item Non-safe primes used with incorrectly chosen $g$:
		      \begin{itemize}
			      \item Servers use short exponents (160--256 bits) for ``efficiency''
			      \item \textbf{AND} generator $g$ generates a small subgroup
			      \item If $g$ generates the full group (order $p-1$), you're still secure!
			      \item But many implementations chose $g$ poorly
		      \end{itemize}
		\item \textbf{Real impact}: 159 servers fully compromised, including VPN devices and web conferencing servers!
	\end{itemize}
\end{frame}
...
\begin{frame}{The Pohlig-Hellman algorithm: divide and conquer}
	\begin{columns}[c]
		...
		\begin{column}{0.46\textwidth}
			\begin{center}
				\begin{tikzpicture}[scale=0.72, every node/.style={align=center}]
...
\begin{frame}{Pohlig-Hellman: a tiny worked example}
	Let $p = 41$: the group order $n = p - 1 = 40 = 2^3 \cdot 5$ is \textbf{smooth}.
	\begin{itemize}[<+->]
		\item Brute force over $\mathbb{Z}_{40}$: up to $40$ checks.
		\item Pohlig-Hellman instead solves $x \bmod 8$ (8 checks) and $x \bmod 5$ (5 checks) separately.
		\item Combine with the \textbf{Chinese Remainder Theorem}: exactly one $x \bmod 40$ matches both.
		\item Total: $\sim 13$ checks instead of $40$ --- and the saving \textbf{explodes} for cryptographic-sized primes.
	\end{itemize}
	\definitionbox{The general rule}{If $p - 1$'s largest prime factor has $b$ bits, Pohlig-Hellman solves the discrete log in $\mathcal{O}(\sqrt{2^b})$ time --- regardless of how big $p$ itself is!}
\end{frame}
...
\begin{frame}{Safe primes: closing the door on Pohlig-Hellman}
	...
	\begin{itemize}[<+->]
		...
		\item $q$ is itself huge, so Pohlig-Hellman gains \textbf{nothing}: the best attack stays $\mathcal{O}(\sqrt{q}) \approx \mathcal{O}(\sqrt{p})$.
		\item Compare an \textbf{unsafe} prime, where $p-1$ is smooth --- exactly what Pohlig-Hellman needs.
	\end{itemize}
	\alertbox{Back to Logjam}{The ``composite-order subgroup attacks'' from Logjam are Pohlig-Hellman in the wild: non-safe primes handed attackers exactly the smooth structure it needs.}
\end{frame}
...
\begin{frame}{WhatsApp: Key transparency}
	\begin{columns}[c]
		\begin{column}{0.52\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{The problem}: Manual QR verification scales terribly.
				      \begin{itemize}
					      \item A group of 100 needs $\binom{100}{2} = \frac{100 \cdot 99}{2} = \textbf{4,950}$ pairwise checks!
				      \end{itemize}
				\item \textbf{WhatsApp (2023)}: \textbf{Key transparency} verifies connections automatically. No user action.\footnote{\url{https://appliedcryptography.page/paper/\#whatsapp-transparency}}
...
\begin{frame}{Modern implementations: elliptic curves}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}[<+->]
				...
				\item 256-bit keys $\approx$ 3072-bit traditional DH
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Same security}: Based on the elliptic curve discrete logarithm problem
...
\begin{frame}{Elliptic curve vs. finite field DH: key sizes}
	\begin{itemize}[<+->]
		\item Baby-step giant-step and Pollard's rho are \textbf{generic}: the same $\mathcal{O}(\sqrt{n})$ attack works in \textbf{any} group of order $n$, whether $\mathbb{Z}_p^*$ or an elliptic curve.
		\item But $\mathbb{Z}_p^*$ has \textbf{extra structure} elliptic curves lack: it also falls to \textbf{index calculus}, a faster, \textbf{subexponential} attack.
	\end{itemize}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{$\mathbb{Z}_p^*$ (finite field DH)}
			\begin{itemize}[<+->]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Elliptic curves (ECDH)}
			\begin{itemize}[<+->]
				\item Only generic $\mathcal{O}(\sqrt{n})$ applies.
				\item Needs just $\sim$256 bits.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{The quantum threat to Diffie-Hellman}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Timeline concerns}:
		      ...
		      \end{itemize}
		\item \textbf{Post-quantum key exchange}: New algorithms are being standardized and deployed.
		      \begin{itemize}
...
\begin{frame}{RSA in practice}
	...
	\begin{itemize}[<+->]
		\item 1024-bit RSA: Deprecated, considered insecure since 2010
...
\begin{frame}{Why we're not covering RSA in depth}
	...
	\begin{itemize}[<+->]
		...
		\item \textbf{Performance issues}: Terribly slow compared to alternatives
		      \begin{itemize}
			      \item 4096-bit RSA signature: $\sim$10ms
			      \item Ed25519 signature: $\sim$0.1ms (100x faster!)
		      \end{itemize}
