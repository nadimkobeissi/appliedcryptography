...
\coversessionname{1.8: Elliptic Curves \&\\Digital Signatures}
...
\begin{frame}{Very intuitively}
	\begin{columns}
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{persistence_of_memory.jpg}{Let's make that structure ``weirder'' using elliptic curves and avoid these attacks.}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{0.6\textwidth}
			...
			\begin{enumerate}[<+->]
				\item \textbf{Verify equation:} Check if:
				      $$S \cdot B = R + \sssubroutinename{hash}{R, A, M} \cdot A$$
				\item \textbf{Accept signature if equation holds}
			\end{enumerate}
			\vspace{0.5cm}
			\textbf{Why this works:}
			\begin{itemize}[<+->]
				\item From signing: $S = r + \sssubroutinename{hash}{R, A, M} \times a$
				\item So: $S \cdot B = (r + \sssubroutinename{hash}{R, A, M} \times a) \cdot B$
				\item $= r \cdot B + \sssubroutinename{hash}{R, A, M} \times a \cdot B$
				\item $= R + \sssubroutinename{hash}{R, A, M} \times A$
			\end{itemize}
		\end{column}
		\begin{column}{0.4\textwidth}
...
\begin{frame}{Choosing the right elliptic curve}
	\begin{itemize}[<+->]
		\item \textbf{Not all elliptic curves are created equal:} Choosing coefficients $a$ and $b$ in $y^2 = x^3 + ax + b$ requires careful consideration.
		...
		\item \textbf{In practice:} You'll use established curves, but understanding what makes a curve safe helps you:
		      ...
		      \end{itemize}
		\item The mathematical structure of the curve directly impacts cryptographic security.
		\item Comprehensive comparison of elliptic curves: \url{https://safecurves.cr.yp.to}
	\end{itemize}
\end{frame}
	...
	\begin{itemize}[<+->]
		\item \textbf{Post-quantum transition:} ECC implementations need migration paths.
		      \begin{itemize}
			      ...
			      \item Discussed in a future class session!
		      \end{itemize}
