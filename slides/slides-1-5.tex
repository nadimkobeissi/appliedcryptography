...
\begin{frame}{\ldots but it can be fragile}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item<1-2> We switch M and R's places in \lib{}{cpa-real}'s encryption function.
	...
	\begin{itemize}
		...
		\item It's brute-force on each byte independently:
		      \begin{itemize}
			      ...
			      \item True brute-force on entire plaintext: $256^n$ (exponentially worse!)
		      \end{itemize}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.35\textwidth}
			\definitionbox{PRF vs PRP}{
				...
				\begin{itemize}
					\item Many inputs can map to same output
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item \textbf{Real-world scenario}:
				      \begin{itemize}
					      \item Bank encrypts transaction records with this scheme.
					      \item Attacker intercepts multiple encrypted transactions.
					      \item By mixing parts, attacker creates valid ciphertexts for transactions that never happened.
					      \item System accepts these as legitimate because they decrypt successfully!
				      \end{itemize}
				\item \textbf{Why CCA security matters}: It ensures that only ciphertexts created by legitimate parties will decrypt successfully.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
			...
			\begin{itemize}
				...
				\item How it works:
				      \begin{itemize}
					      ...
					      \item Evaluates polynomial at secret point $H = \texttt{AES}(K, 0^{128})$
	...
	\begin{columns}[c]
			...
			\begin{itemize}
				\item \textbf{Why not use HMAC-SHA256 or similar?}
				      \begin{itemize}
					      ...
					      \item HMAC requires multiple passes over the data
		...
		\begin{column}{0.35\textwidth}
			\definitionbox{Performance Comparison}{
				For 1KB message:
				\begin{itemize}
					\item \textbf{HMAC-SHA256}: ~3000 cycles
					\item \textbf{GHASH (software)}: ~1500 cycles
					\item \textbf{GHASH (hardware)}: ~200 cycles
				\end{itemize}
				\vspace{0.5em}
				GHASH can be 15x faster with hardware support!
			}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item Practical impact:
				      \begin{itemize}
					      \item Attacker can create ciphertexts that decrypt differently under different keys.\footnote{\url{https://appliedcryptography.page/paper/\#key-commitment}}
