...
\begin{frame}{Non-deterministic encryption isn't hard\ldots}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item AES-CTR turns AES into non-deterministic encryption\ldots
				\item AES-GCM even turns it into a non-deterministic authenticated cipher\ldots\footnote{More on these later. An authenticated cipher is one where the ciphertext can't be modified by the adversary without detection.}
				\item We can just as easily build non-deterministic encryption from any PRF:
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
...
\begin{frame}{\ldots but it can be fragile}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item<1-2> We swap the positions of $M$ and $R$ in \lib{}{cpa-real}'s encryption function.
		...
		\begin{column}{0.5\textwidth}
			\sslinked{
				\ssprogram{}{
					...
					return $S_1 \oplus S_2 \stackrel{?}{=} R_1 \oplus R_2$
				}{0.8}
			}{\link}{
				\sslibrary{}{cpa-real}{
					$K \twoheadleftarrow \bits^\lambda$\\[1em]
...
\begin{frame}{Understanding the CCA-rand library}
	\begin{itemize}
		\item The \lib{\Sigma}{cca-rand} library represents the \textbf{ideal} CCA-secure encryption:
		      \begin{itemize}
			      \item \func{cca.enc}{} returns completely random ciphertexts.
			      \item The library secretly remembers what plaintext belongs to each ciphertext it created (stored in $\mathcal{D}$).
			      \item \func{cca.dec}{} only returns the correct plaintext for ciphertexts it created itself.
		      \end{itemize}
...
\begin{frame}{Malleability once again}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\end{itemize}
			\imagewithcaption{prf-enc-malleable}{Source: The Joy of Cryptography}
		\end{column}
		\begin{column}{0.5\textwidth}
			\ssprogram{}{
				$M \twoheadleftarrow \bits^\lambda$\\
				$\Delta \coloneq \text{arbitrary nonzero $\lambda$-bit string}$\\
...
\begin{frame}{Evaluating our distinguisher}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{center}
				\sslinked{
...
\begin{frame}{Another example}
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			...
			\end{itemize}
			\imagewithcaption{prf-enc-2}{Source: The Joy of Cryptography}
		\end{column}
		\begin{column}{0.4\textwidth}
			\sslibrary{}{cca-real}{
				$K \twoheadleftarrow \bits^\lambda$\\[1em]
...
\begin{frame}{Wait, what's $F^{-1}$?}{Reminder: PRFs vs. PRPs}
	\begin{columns}[c]
		...
		\begin{column}{0.35\textwidth}
			\definitionbox{PRF vs. PRP}{
				\textbf{PRF}: $F: \mathcal{K} \times \mathcal{X} \to \mathcal{Y}$
				\begin{itemize}
					\item Many inputs can map to the same output
					\item Not invertible in general
				\end{itemize}
				\vspace{0.5em}
				\textbf{PRP}: $F: \mathcal{K} \times \mathcal{X} \to \mathcal{X}$
				\begin{itemize}
					\item For each key, $F(K, \cdot)$ is a permutation
					\item Invertible: $F^{-1}$ exists
				\end{itemize}
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Another example}
	\begin{columns}[c]
		\begin{column}{0.35\textwidth}
			\begin{itemize}
				\item When linked to \lib{}{cca-real}:
				\item \prob{\prog{} \Rightarrow \texttt{true}} $ = 1$
				\item When linked to \lib{}{cca-rand}:
				\item \prob{\prog{} \Rightarrow \texttt{true}} $\approx \frac{1}{2^\lambda}$
			\end{itemize}
		\end{column}
		\begin{column}{0.65\textwidth}
			\sslinked{
...
\begin{frame}{Why does the Frankenstein attack work?}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item When linked to \lib{}{cca-real}:
				\item Both $R_1\|S_1$ and $R_2\|S_2$ encrypt the same $M$:
				      \begin{itemize}
					      \item $S_1 = F(K, R_1) \oplus F(K, M)$
					      \item $S_2 = F(K, R_2) \oplus F(K, M)$
				      \end{itemize}
				\item Decrypting $R_1\|S_2$:
				      \begin{itemize}
					      \item $M_1^* = F^{-1}(K, F(K, R_1) \oplus S_2)$
					      \item $= F^{-1}(K, F(K, R_1) \oplus F(K, R_2) \oplus F(K, M))$
				      \end{itemize}
				\item Decrypting $R_2\|S_1$:
				      \begin{itemize}
					      \item $M_2^* = F^{-1}(K, F(K, R_2) \oplus S_1)$
					      \item $= F^{-1}(K, F(K, R_2) \oplus F(K, R_1) \oplus F(K, M))$
				      \end{itemize}
				\item Both equal the same value!
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{Why does the Frankenstein attack matter?}
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item This can have practical implications:
				\item \textbf{Message integrity violation}:
				      \begin{itemize}
					      \item Adversary can create new valid ciphertexts without knowing the key.
					      \item These ``Frankenstein'' ciphertexts decrypt to valid (but unintended) plaintexts.
				      \end{itemize}
				\item \textbf{Real-world scenario}:
				      \begin{itemize}
					      \item A bank encrypts transaction records with this scheme.
					      \item An attacker intercepts multiple encrypted transactions.
					      \item By mixing parts, the attacker creates valid ciphertexts for transactions that never happened.
					      \item The system accepts these as legitimate because they decrypt successfully!
				      \end{itemize}
				\item \textbf{The fix}: we need schemes where only ciphertexts created by legitimate parties will decrypt successfully - as we'll see, this is exactly what authenticated encryption provides.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
			...
			\begin{itemize}
				...
				\item How it works:
				      \begin{itemize}
					      ...
					      \item Evaluates the polynomial at a secret point $H = \texttt{AES}(K, 0^{128})$.
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{GHASH limitations:}
				      \begin{itemize}
					      \item \textbf{Requires a secret key} - GHASH is a keyed function, not suitable for public verification.
				...
				\item \textbf{When we need SHA-2/SHA-3:}
				      \begin{itemize}
					      ...
					      \item \textbf{Password hashing}: no secret key available
