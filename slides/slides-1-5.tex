		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item If the same message always encrypts to the same ciphertext:
				      \begin{itemize}
					      \item Attackers can recognize repeated messages.
	...
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\begin{itemize}[<+->]
				...
				\item Sequential encryption (\textbf{can't parallelize}).
			\end{itemize}
		\end{column}
		\begin{column}{0.4\textwidth}
...
\begin{frame}{Why is it called \func{nulloracle}{C}?}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			\vspace{-0.4cm}
			\bigimagewithcaption{oracle-painting-zoom.jpg}{\textit{The Oracle} by Camillo Miola, 1880\footnote{Absolutely beautiful painting, well worth viewing in high resolution: \url{https://upload.wikimedia.org/wikipedia/commons/9/94/Camillo_Miola_\%28Biacca\%29_-_The_Oracle_-_72.PA.32_-_J._Paul_Getty_Museum.jpg}}}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{itemize}
		...
		\item For a 16-byte message:
		      \begin{itemize}
			      ...
			      \item True brute-force: $\approx 10^{38}$ queries ($256^{16}$)
		      \end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item The ideal \lib{\Sigma}{cca-rand} decrypts only what it created (via dictionary $\mathcal{D}$); tampered ciphertexts fall through to the real \texttt{Dec}, so any information the scheme leaks on them becomes visible to the adversary.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\sslinked{
			...
			\begin{itemize}
				...
				\item Suppose a bank encrypts transactions with the Frankenstein scheme:
				      \begin{itemize}
					      ...
					      \item Mixing their halves forges \emph{valid} ciphertexts for transactions that never happened.
					      \item The system decrypts them without error and accepts them.
				      \end{itemize}
...
\begin{frame}{Combining MACs and encryption}
	\begin{itemize}
		...
		\item There are three common approaches to combining them:
		      ...
		      \end{itemize}
		\item Only one approach guarantees CCA security when using any CPA-secure encryption scheme.
	\end{itemize}
\end{frame}
			...
			\begin{itemize}
				...
				\item \textbf{Encryption process}:
				      \begin{itemize}
					      ...
					      \item GHASH (Galois field\footnote{``Galois field'' is another name for finite fields. We use those for Diffie-Hellman too, remember?} multiplication) for authentication.
				      \end{itemize}
	...
	\begin{columns}[c]
		\begin{column}{0.7\textwidth}
			\begin{itemize}
				...
				\item Key properties:
				      \begin{itemize}
					      \item Operates in the finite field $GF(2^{128})$.
					      \item Uses polynomial multiplication modulo an irreducible polynomial.
					      \item Extremely fast with hardware support.
				      \end{itemize}
				\item How it works:
				      \begin{itemize}
		...
		\begin{column}{0.65\textwidth}
			\begin{itemize}
				...
				\item \textbf{Mathematical elegance:}
				      \begin{itemize}
					      \item Galois field operations have nice algebraic properties.
					      \item Security proofs are cleaner.
				      \end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.35\textwidth}
			\definitionbox{Performance Comparison}{
				For a 1KB message:
				\begin{itemize}
					\item \textbf{HMAC-SHA256}: $\sim$3,000 cycles
					\item \textbf{GHASH (software)}: $\sim$1,500 cycles
					\item \textbf{GHASH (hardware)}: $\sim$200 cycles
				\end{itemize}
				\vspace{0.5em}
				GHASH can be 15$\times$ faster with hardware support!
			}
		\end{column}
	\end{columns}
\end{frame}
		...
		\begin{column}{1\textwidth}
			\begin{itemize}
				\item \textbf{GHASH limitations:}
				      \begin{itemize}
					      \item \textbf{Requires a secret key} - GHASH is a keyed function.
					      \item \textbf{Not collision-resistant} without the key - anyone can find collisions trivially.\footnote{\url{https://appliedcryptography.page/paper/\#universal-hash}}
					      \item \textbf{Linear structure} - $\text{GHASH}_H(A \oplus B) = \text{GHASH}_H(A) \oplus \text{GHASH}_H(B)$.
					      \item \textbf{Forgeable if key is known} - completely breaks down with key exposure.
				      \end{itemize}
				\item \textbf{When we need SHA-2/SHA-3:}
				      ...
				      \end{itemize}
				\item \textbf{Different tools for different jobs:}
				      \begin{itemize}
					      \item GHASH: Fast MAC when you have a secret key
					      \item SHA-2/3: Collision-resistant hash when you need public verifiability
				      \end{itemize}
			\end{itemize}
		\end{column}
	\end{columns}
	...
	\begin{columns}[c]
		\begin{column}{1\textwidth}
			\begin{itemize}
				...
				\item Most AEAD schemes (including AES-GCM) don't guarantee this property!
				\item Attack scenario:
				      \begin{enumerate}
					      \item The attacker creates a special ciphertext $C$.
				...
				\item Practical impact:
				      \begin{itemize}
					      \item An attacker can create ciphertexts that decrypt differently under different keys.\footnote{\url{https://appliedcryptography.page/paper/\#key-commitment}}
...
\begin{frame}{Replay attacks}{Step 1: Alice sends original message}
	\bigimagewithcaption{aead-scenario-1.pdf}{Source: The Joy of Cryptography}
\end{frame}

\begin{frame}{Replay attacks}{Step 2: Attacker replays message in a different context}
	\bigimagewithcaption{aead-scenario-2.pdf}{Source: The Joy of Cryptography}
\end{frame}
...
\begin{frame}{Solution: use associated data to provide context}
	\bigimagewithcaption{aead-scenario-4.pdf}{Source: The Joy of Cryptography}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.4\textwidth}
			\definitionbox{FO Transform (simplified)}{
				Given a CPA-secure PKE scheme $\Pi$:
				\begin{itemize}
					\item $\texttt{Enc}_{FO}(pk, M)$:
					      ...
					      \end{enumerate}
					\item Decryption verifies consistency by re-encrypting $M$ with $H(M)$
				\end{itemize}
			}
		\end{column}
	\end{columns}
