...
\begin{frame}{The problem: trust in digital commerce}
	\begin{itemize}
		...
		\item Banks, payment processors, and credit card companies mediate every transaction.
		\item This trust-based model has fundamental limitations:
		      \begin{itemize}
			      ...
			      \item Transaction costs increase significantly.
		      \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Weaknesses of the trust-based model}
	\begin{itemize}
		...
		\item \textbf{Fraud is inevitable}: Some percentage of fraud is accepted as a cost of doing business.
...
\begin{frame}{Bitcoin's solution}
	\begin{itemize}
		...
		\item Security assumption: Honest nodes control a majority of CPU power.
...
\begin{frame}{Historical context}
	\begin{itemize}
		...
		\item Built on decades of cryptographic research:
		      ...
		      \end{itemize}
		\item First practical solution to the Byzantine Generals Problem for money.
	\end{itemize}
\end{frame}
...
\begin{frame}{The Byzantine Generals Problem}
	\begin{itemize}
		...
		\item Scenario: Byzantine generals must coordinate an attack on a city.
		      \begin{itemize}
			      \item Generals can only communicate via messengers.
			      \item Some generals may be traitors sending false messages.
			      \item Need consensus: all loyal generals execute the same plan.
		      \end{itemize}
...
\begin{frame}{Proof-of-Work: A puzzle anyone can solve (but slowly)}
	...
	\begin{alertblock}{Key property}
		You prove you spent real resources, and anyone can verify your proof cheaply
	\end{alertblock}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{exampleblock}{Why hashing?}
				Each attempt is independent - can't get ``closer'' to a solution, only keep trying
			\end{exampleblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{alertblock}{Safety limit}
				Maximum adjustment: 4$\times$ up or down per period (prevents manipulation)
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{exampleblock}{The reward}
				Winner gets to create new bitcoins (currently 3.125 BTC $\approx$ \$200K) plus transaction fees
			\end{exampleblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item 1000 fake identities with 1 hash/sec each are no better than 1 identity with 1000 hash/sec
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item To rewrite history:
				      ...
				      \end{itemize}
				\item Success probability shrinks exponentially!
			\end{itemize}
...
\begin{frame}{The elegant lottery mechanism}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Every attempt has the same probability
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Electricity (biggest cost!)
				      \begin{itemize}
					      \item Often exceeds the hardware's cost over its lifetime
				      \end{itemize}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{exampleblock}{Equilibrium}
				Total mining cost $\approx$ Total rewards\\
...
\begin{frame}{The environmental elephant in the room}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{The scale:}
			\begin{itemize}
				\item Peak (2021): $\approx$180 TWh/year
...
\begin{frame}{Alternative PoW designs: Can we do better?}
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Required a 4 GB dataset in RAM
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item The losing block becomes ``orphaned''
			\end{itemize}
...
\begin{frame}{The 51\% attack: The one big vulnerability}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{What if an attacker gets >50\% hashpower?}
			\begin{itemize}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Would crash Bitcoin's price
			...
			\begin{itemize}
				...
				\item Transaction:
				      \begin{itemize}
					      ...
					      \item Implicit fee: 0.1 BTC (input $-$ outputs)
				      \end{itemize}
			\end{itemize}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Could send one coin to two people!
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Each block hashes the previous block
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Proof-of-work proves the timestamp
			\end{itemize}
...
\begin{frame}{Transaction confirmation in practice}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{The math behind it:}\footnote{\url{https://bitcoin.org/bitcoin.pdf}, section 11}
			\begin{itemize}
...
\begin{frame}{The blockchain scaling problem}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{The challenge:}
			\begin{itemize}
				\item A block can have 2000+ transactions
				\item Full block: $\approx$1--2 MB
				\item New block every 10 minutes
				\item 850,000+ blocks so far
				\item Total: over 500 GB!
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
...
\begin{frame}{Merkle trees: Bitcoin's elegant solution}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Build a \textbf{tree of hashes}
				\item Put a single root in the block header
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Block with 2000 txs: $\approx$1 MB
				\item Block header: 80 bytes
				\item Merkle proof: $\approx$400 bytes
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Logarithmic verification time
...
\begin{frame}{Security properties in detail}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item From SHA-256
				\item Can't find two transactions with the same hash
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Commitment:}
			...
			\begin{itemize}
				\item Can't find a different tx with the same proof path
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{alertblock}{Security model}
				SPV additionally trusts majority hashpower for transaction validity - full nodes verify validity themselves!
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Example: Git is a blockchain!}
	\begin{columns}[c]
			...
			\begin{itemize}
				\item Each commit has:
				      \begin{itemize}
					      ...
					      \item Optional author signature
				      \end{itemize}
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Git: Usually centralized (GitHub)
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{exampleblock}{Brilliance}
				Solves the halting problem with economics!
			\end{exampleblock}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{How do you get gas? Understanding the gas market}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{You don't ``buy'' gas directly:}
			\begin{itemize}
				\item Gas is just a unit of measurement
				\item Like ``miles'' or ``kilowatts''
				\item You pay for gas in ETH
			\end{itemize}
			\textbf{How it works:}
			\begin{itemize}
				\item Have ETH in your account
				\item Send transaction
				\item Specify gas price you'll pay
				\item Miners/validators choose which txs to include
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{What sets gas prices?}
			\begin{itemize}
				\item \textbf{Supply \& demand!}
				\item Network congested? Prices up
				\item Quiet period? Prices down
				\item Users bid for block space
			\end{itemize}
			\begin{alertblock}{Auto-suggestion algorithm}
				Wallets automatically suggest a gas price based on the current market!
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}

\begin{frame}{The gas auction: Priority fees}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Post-EIP-1559 (2021):}
			\begin{itemize}
				\item Base fee (burned)
				      \begin{itemize}
					      \item Set by protocol
					      \item Adjusts with demand
				      \end{itemize}
				\item Priority fee (tip)
				      \begin{itemize}
					      \item Goes to validator
					      \item You choose amount
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item Total fee = (Base fee + Priority fee) $\times$ Gas used
				\item Base fee: 30 gwei
				\item Your tip: 2 gwei
	...
	\begin{exampleblock}{Market dynamics}
		High demand $\rightarrow$ Base fee increases\\
		Want fast tx $\rightarrow$ Increase tip
	\end{exampleblock}
\end{frame}
...
\begin{frame}{Decentralized Autonomous Organizations (DAOs)}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{What is a DAO?}
			\begin{itemize}
				\item Organization governed by code
				\item Rules encoded in smart contracts
				\item No central authority
				\item Token holders vote on decisions
				\item Treasury controlled by members
			\end{itemize}
			\textbf{How they work:}
			\begin{itemize}
				\item Members hold governance tokens
				\item Proposals submitted on-chain
				\item Token-weighted voting
				\item Automatic execution if passed
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Examples:}
			\begin{itemize}
				\item MakerDAO: Manages DAI stablecoin
				\item Uniswap DAO: Protocol governance
				\item ConstitutionDAO: Attempted to buy a copy of the US Constitution (\$47M raised!)
			\end{itemize}
			\textbf{Use cases:}
			\begin{itemize}
				\item Investment clubs
				\item Protocol governance
...
\begin{frame}{The DAO hack: When code goes wrong}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Largest crowdfund ever at the time
	...
	\begin{alertblock}{Lesson}
		Code is law... but bugs are expensive! Led to the Ethereum hard fork.
	\end{alertblock}
\end{frame}
...
\begin{frame}{NFTs: Non-Fungible Tokens}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{What are NFTs?}
			\begin{itemize}
				\item Non-fungible = unique, not interchangeable
				\item Token standard (ERC-721)
				\item Proves ownership of digital asset
				\item Metadata points to content
			\end{itemize}
			\textbf{How they work:}
			\begin{itemize}
				\item Smart contract tracks ownership
				\item TokenID uniquely identifies each NFT
				\item Transfer ownership on-chain
				\item Anyone can verify authenticity
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Use cases:}
			\begin{itemize}
				\item Digital art (Beeple: \$69M sale)
				\item Profile pictures (Bored Apes)
				\item Gaming items
				\item Event tickets
				\item Domain names (ENS)
			\end{itemize}
			\textbf{The controversy:}
			\begin{itemize}
				\item Speculation bubble?
				\item Right-click save problem
				\item Environmental concerns (pre-Merge)
				\item What are you really buying?
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{The Merge: September 2022}
	...
	\end{columns}
	\begin{center}
		\textbf{From millions of households' worth of energy to just a few thousand}
	\end{center}
\end{frame}
...
\begin{frame}{What incentivizes honest behavior?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{Carrots (rewards):}
			\begin{itemize}
				\item Attestation rewards
				      \begin{itemize}
					      \item Vote on correct chain head
					      \item $\approx$0.003 ETH per day
				      \end{itemize}
				\item Block proposal rewards
				      \begin{itemize}
					      \item Randomly selected
					      \item Transaction fees + tips
					      \item Can be substantial
				      \end{itemize}
				\item Sync committee rewards
				      \begin{itemize}
					      \item Help light clients
					      \item Additional yield
				      \end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Sticks (penalties):}
			\begin{itemize}
				\item Inactivity leaks
				      \begin{itemize}
					      \item Offline? Lose rewards
					      \item Proportional to downtime
				      \end{itemize}
				\item Slashing
				      \begin{itemize}
					      \item Double-sign? Lose 1+ ETH
					      \item Forced exit from validator set
					      \item Slashed funds burned
				      \end{itemize}
				\item Opportunity cost
				      \begin{itemize}
					      \item Capital locked up
					      \item Acting honestly more profitable
				      \end{itemize}
			\end{itemize}
		\end{column}
	\end{columns}
	\begin{center}
		\textbf{Expected return: $\approx$4--5\% APR for honest validators}
	\end{center}
\end{frame}
...
\begin{frame}{How is dishonesty detected?}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\textbf{The watchers:}
			\begin{itemize}
				\item Every validator watches every other validator
				\item All attestations are public
				\item All blocks are public
			\end{itemize}
			\textbf{Slashable offenses:}
			\begin{itemize}
				\item \textbf{Double proposal:}
				      \begin{itemize}
					      \item Propose 2 blocks at same height
					      \item Trying to fork the chain
				      \end{itemize}
				\item \textbf{Surround vote:}
				      \begin{itemize}
					      \item Attest to conflicting checkpoints
					      \item Violates finality rules
				      \end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			...
			\begin{enumerate}
				\item Validator misbehaves
				\item Creates cryptographic evidence
				      \begin{itemize}
					      \item Two signed messages
					      \item Both with validator's signature
					      \item Provably conflicting
				      \end{itemize}
				\item Anyone can submit proof
				\item Proof included in block
				\item Slashing automatically executed
			\end{enumerate}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Critiques of Proof-of-Stake}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				\item In PoW, mining on multiple chains wastes energy
				\item In PoS, validating multiple chains is free
				\item Could validators hedge by validating all forks?
				\item Mitigation: Slashing for double-signing
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Wealth concentration:}
			\begin{itemize}
				\item Rich get richer (staking rewards)
				\item High barrier to entry (32 ETH)
				\item Centralization concern
				\item Counter: Staking pools, but they add trust
			\end{itemize}
			\begin{alertblock}{The debate}
				Is economic security as strong as energy security?
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Post results to an Ethereum smart contract
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{exampleblock}{The result}
				100$\times$ cheaper fees, 100$\times$ higher throughput, same security!
			\end{exampleblock}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Challenges ahead}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}
				...
				\item Moving between L2s is hard
	...
	\vspace{1em}
	\begin{center}
		\textbf{The ecosystem is evolving fast. L2s are a path to scaling Ethereum while preserving what makes it special.}
	\end{center}
\end{frame}
