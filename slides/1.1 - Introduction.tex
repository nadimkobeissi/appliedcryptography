...
\coversessionname{1.1: Introduction}
...
\begin{document}
		...
		\begin{columns}[c]
			\begin{column}{0.5\textwidth}
				\begin{itemize}
					...
					\item \textbf{Cryptography}: \textit{"the practice and study of techniques
						for secure communication in the presence of adversarial behavior"} —
						Ronald Rivest \pause

					\item Necessary since the dawn of civilization. Example: \textbf{Caesar
						cipher}.
				\end{itemize}
			\end{column}
			...
			\begin{column}{0.5\textwidth}
				...
				\begin{itemize}
					...
					\item \textbf{Confidentiality}: Data exchanged between Client and Server
						is only known to those parties. \pause

					\item \textbf{Authentication}: If Server receives data from Client,
						then Client sent it to Server. \pause
			...
			\begin{column}{0.5\textwidth}
				...
				\begin{itemize}
					...
					\item \textbf{Confidentiality}: When you send a private message on Signal,
						only you and the recipient can read the content. \pause

					\item \textbf{Authentication}: When you receive an email from your boss,
						you can verify it actually came from them. \pause
	...
	\begin{frame}{Security goals: more examples}
		\begin{itemize}
			...
			\item \textbf{Signal} implements post-compromise security, an advanced security
				goal.
				\begin{itemize}
					\item Allows a conversation to "heal" in the event of a temporary key compromise.
	...
	\begin{frame}{"Hard problems"}
		\begin{itemize}
			...
			\item With cryptography, we get security systems that we can literally
				mathematically prove as secure (under assumptions). \pause
			...
			\item Also, this allows for actual magic.
				\begin{itemize}
					...
					\item You are listening to everything they are saying. \pause
		...
		\begin{columns}[c]
			\begin{column}{0.5\textwidth}
				...
				\begin{itemize}
					...
					\item Diffie-Hellman, RSA, ML-KEM, etc. \pause

					\item "Asymmetric" because there is a "public key" and a "private key"
						for each party. \pause
			...
			\begin{column}{0.5\textwidth}
				...
				\begin{itemize}
					...
					\item AES, SHA-2, ChaCha20, HMAC\ldots \pause

					\item "Symmetric" because there is one secret key. \pause

					\item Not algebraic but unstructured, but on their understood
						resistance to $n$ years of cryptanalysis. \pause
	...
	\begin{frame}{Kerckhoff's Principle}
		\begin{itemize}
			\item \textit{"A cryptosystem should be secure even if everything about the
				system, except the key, is public knowledge."} — Auguste Kerckhoffs, 1883
				\pause
			...
			\item \textbf{Why it matters}:
				\begin{itemize}
					...
					\item No "security through obscurity" \pause

					\item The key is the only secret: the rest can be audited, tested,
						trusted \pause

					\item Encourages open standards and peer review \pause
	...
	\begin{frame}{Symmetric primitive example: hash functions}
		\begin{itemize}
			...
			\item $\mathsf{SHA256}(\texttt{"hello"}) = \texttt{0x2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824
				}$ \pause

			\item $\mathsf{SHA256}(\texttt{"hullo"}) = \texttt{0x7835066a1457504217688c8f5d06909c6591e0ca78c254ccf17450d0d999cab0
				}$ \pause

			\item $\mathsf{SHA256}(\textit{veryLongInputOfAMillionBytes}) = \texttt{0x2be5b2bcfbe63c4ba5ea9acd0e6a8d714bff843a2dbf0ef0abcfe7487e5e83cd
				}$
		\end{itemize}
	\end{frame}
				...
				\begin{itemize}
					...
					\item Not that far off in terms of design process from hash functions,
						...
						\begin{itemize}
							...
							\item SHA-2 is a PRF (pseudorandom function)
						\end{itemize}
				\end{itemize}
			\end{column}
		...
		\begin{columns}[c]
			...
			\begin{column}{0.8\textwidth}
				\begin{tikzpicture}[scale=0.38]
					% Define colors
					\definecolor{domaingreen}{RGB}{102, 170, 68}
					\definecolor{rangegreen}{RGB}{102, 170, 68} % same color as domain
					\definecolor{circlecolor}{RGB}{235, 137, 85}
					\definecolor{purplearrow}{RGB}{160, 78, 160}
	...
	\begin{frame}{TLS 1.3: high-level skwtch}
	...
	\begin{frame}{Class materials}
		\begin{itemize}
			\item Joy of Cryptography: learn about the math, how to reason about and prove
				systems secure.

			\item Serious Cryptography: hard engineering perspective.
				\vspace{1cm}

			\item Many assignments!

			\item Many lab sessions!
