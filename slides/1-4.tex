...
\begin{frame}{The Golden Rule of PRFs}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\alertbox{Attack Strategy}{
				To attack $H$, choose inputs that will cause $F$ to be called on a repeated input.
			}
			\begin{itemize}[<+->]
			...
			\end{itemize}
			\vspace{1em}
			\alertbox{Proof Strategy}{
				To prove security of $H$, establish that inputs to $F$ do not repeat, except with negligible probability.
			}
		\end{column}
	\end{columns}
\end{frame}
