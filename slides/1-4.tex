	...
	\begin{columns}[c]
		...
		\begin{column}{0.55\textwidth}
			\begin{flushright}
				\sslinked{
					\ssprogram{}{
						$A \| B \| C \| D \coloneq \sssubroutinename{ots.enc}{}$ \\
						return $G(B) == C \| D$
					}{0.9}
				}{\link}{
					\sslibrary{H}{prg-real}{
...
\begin{frame}{Building a permutation through Feistel ciphers}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{feistel_cipher.png}{Feistel cipher.\\Source: The Joy of Cryptography}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Building a permutation through Feistel ciphers}{Inversion}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.5\textwidth}
			\imagewithcaption{feistel_cipher_inv.png}{Feistel cipher inversion.\\Source: The Joy of Cryptography}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Meet-in-the-middle attacks on Feistel ciphers}
	\begin{columns}[c]
		\begin{column}{0.65\textwidth}
			\begin{itemize}[<+->]
				\item Using the \textbf{same key} for all rounds creates significant vulnerabilities.
				\item A meet-in-the-middle attack can break an $r$-round Feistel cipher with complexity much less than $2^r$.
				\item The attack works by computing partial encryptions from both ends:
				      ...
				      \end{enumerate}
				\item For a Feistel cipher with identical round functions, the effective security may be only about \textbf{half} the number of rounds.
			\end{itemize}
		\end{column}
	...
	\begin{columns}[c]
		\begin{column}{0.4\textwidth}
			\begin{itemize}[<+->]
				\item Let's query $(A \| B)$ and $(A' \| B)$ where $A \neq A'$
				      \begin{itemize}
					      \item $Y_1 \| Y_2 = \sssubroutinename{prp.query}{A \| B}$
					      \item $Y_1' \| Y_2' = \sssubroutinename{prp.query}{A' \| B}$
				      \end{itemize}
		...
		\begin{column}{0.6\textwidth}
			\begin{center}
				\sslinked{
				\ssprogram{}{
				$Y_1\|Y_2 = \sssubroutinename{prp.query}{A\|B}$\\
				$Y^{'}_1\|Y^{'}_2 = \sssubroutinename{prp.query}{A^{'}\|B}$\\
...
\begin{frame}{However, a 3+ round Feistel cipher is fine!}
	\begin{columns}[c]
		\begin{column}{0.4\textwidth}
			\begin{itemize}[<+->]
				\item Luby and Rackoff proved that a 3-round Feistel cipher is indistinguishable from a pseudorandom permutation.\footnote{Link to the paper is available on the course website at \url{https://appliedcryptography.page}, under \textit{Online Readings}.}
...
\begin{frame}{The ``bad event'' proof technique}{Reminder}
	\definitionbox{Bad Event Technique}{
		...
		\begin{align*}
			\left| \prob{\prog{} \link \lib{}{1} \Rightarrow \texttt{true}} - \prob{\prog{} \link \lib{}{2} \Rightarrow \texttt{true}} \right| \leq \prob{\prog{} \link \lib{}{1} \sssubroutinename{trigger}{\texttt{bad}}}
		\end{align*}
	}
\end{frame}
...
\begin{frame}{The ``bad event'' proof technique}{Reminder}
	\begin{itemize}[<+->]
		\item \prog{}'s advantage is bounded by \prob{\prog{} \link \lib{}{1} \sssubroutinename{trigger}{\texttt{bad}}}.
...
\begin{frame}{DES: Feistel in practice}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{Data Encryption Standa.rd (DES)} is a classic real-world implementation of the Feistel structure
				\item Properties:
				      \begin{itemize}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}[<+->]
				\item \textbf{Triple DES (3DES)} replaced it:
				      ...
				      \end{itemize}
				\item 3DES is still used in some legacy systems, but has largely been replaced by AES for performance reasons.
				\item Lesson: Feistel structure made it possible to adapt DES rather than completely abandon it.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
